<template>
  <div class="aop_tradedesign_comps_d4_edit_biz_rule_inner_table_select_dict">
    <PageDialog
      :dialogWidth="'90%'"
      :dialogTitle="'请选择参数'"
      ...
      class="aop_tradedesign_comps_d4_edit_biz_rule_inner_table_select_dict_append_to_bodye_dialog"
    >
      <div slot="box">
...
export default {
  ...
  methods: {
    async showDialog(dataSetInfo) {
      console.log("showDialog", dataSetInfo)
      ...
      this.getDataParamsList(dataSetInfo);
    },
    closeDialog() {
    ...
    confirm() {
      if (!this.radioModel) return this.$message.error("请选择参数")
      // this.$emit("setOutputTarget", "column", this.currentColumnObj, this.paramsDetail)
      this.$emit("addColumn", this.paramsDetail)
      this.closeDialog()
    },
   
    //获取分段
    async getDataParamsList(dataSetInfo) {
      const params = {
        dataSetNo: dataSetInfo.dataSetNo || "",
        currentPage: "1",
      ...
      if (res.dataSubsectionList.length > 0) {
        ...
        this.menuDatasetList[0].dataSubstnNo = dataSetInfo.dataSetNo || ""
        this.menuDatasetList[0].dataSubstnNm = dataSetInfo.dataSetNm || ""
    ...
    //字段单选调接口L5服务
    radioChange(e) {
      this.radioModel = e.dataSetDataId;
      this.paramsDetail = e
    },
...
.aop_tradedesign_comps_d4_edit_biz_rule_inner_table_select_dict {
  .container {
    padding: 16px;
  }
  .edit_form {
...
.aop_tradedesign_comps_d4_edit_biz_rule_inner_table_select_dict_append_to_bodye_dialog {
  .el-dialog__body {
    padding: 0!important;
  }
  .add_dialog_conf1 {
