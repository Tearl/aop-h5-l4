<template>
  <div class="aop_tradedesign_comps_process">
    <div ref="svgBox" class="process_svg_box">
      ...
      >
        ...
        >
          ...
          <template v-if="['cacheSessionAction', 'getSessionAction', 'createGraphValidateCodeAction', 'verifyGraphValidateCodeAction'].includes(item.actionType)">
            <rect
              :x="item.actionX"
              :y="item.actionY"
              :width="getItemWidth(item)"
          ...
          <foreignObject
            v-if="['cacheSessionAction', 'getSessionAction', 'createGraphValidateCodeAction', 'verifyGraphValidateCodeAction', 'getCacheAction', 'setCacheAction'].includes(item.actionType)"
            :x="item.actionX"
            :y="item.actionY"
            :width="getItemWidth(item)"
            :height="getItemHeight(item)"
          ...
          >
            <p class="d_rect_text">
              <i v-if="['cacheSessionAction', 'getSessionAction', 'createGraphValidateCodeAction', 'verifyGraphValidateCodeAction'].includes(item.actionType)" class="icon el-icon-user"></i>
              <i v-else-if="['getCacheAction', 'setCacheAction'].includes(item.actionType)" class="icon el-icon-coin"></i>
              {{ getDServText(item, "actionSeq") || "位置序号" }}
            </p>
            <p class="d_rect_code" :class="{
              blue: ['cacheSessionAction', 'getSessionAction', 'createGraphValidateCodeAction', 'verifyGraphValidateCodeAction', 'getCacheAction', 'setCacheAction'].includes(item.actionType)
            }">
              {{ getDServText(item, "actionDesc") }}
            </p>
            <p class="d_rect_msg mt5">
    ...
    */
    ...
    handleItemClick(e, click = true) { // 默认点击触发，拖拽组件触发时click = false
      ...
      this.handleItemClick.timer = setTimeout(() => {
        // if (this.isRead) return;
        // 当isTest为true时，则为服务资产预览执行测试页面，组件不可移动
        if (this.isTest) {
    ...
    upDateServParams(obj, e) {
      ...
      } else if (["cacheSessionAction", "getSessionAction"].includes(obj.actionType)) {
        const target = this.dataList.find(item => item.actionId == obj.actionId)
        target.paramList = e.paramList
        target.conf = e.confList.map(item => {
          return {
