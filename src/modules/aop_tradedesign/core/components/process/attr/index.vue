<template>
  <div class="aop_tradedesign_comps_d5design_process_attr" :style="{flexBasis: flexBasis, maxWidth: flexBasis, userSelect: userSelect }">
    <div 
      class="header_close" 
  ...
  methods: {
      ...
      // setTimeout(() => {
        new Promise((resolve, reject) => {
          ...
          if (["D4bAction", "D5SelfAction", "D5LinkAction"].indexOf(e.actionType) > -1) {
            ...
            this.$nextTick(() => {
              this.$refs.operationAttrRef.getParams(e, this.currentServObj)
            })
    ...
    upDateServParams(currentObj, e, layoutServiceBeanList) {
      ...
      this.getToSave().then(()=> {
        ...
        if (["loopAction"].includes(currentObj.actionType)) {
    ...
    async getToSave() {
      const params = {
        flowId: this.fourRAttrFlag ? this.currentObj.compositeId : this.transId,
    ...
    mouseDown(ev) {
      let x = ev.clientX;
      let s = document.querySelector(".aop_tradedesign_comps_d5design_process_attr");
    ...
    async getAllL5ServiceParamList() {
      ...
      const promises = dataList.reduce((prev, data) => {
        if (["D4bAction", "D5SelfAction", "D5LinkAction"].includes(data.actionType) && !(data.actionOwnId in this.serviceIdMapToParamList)) {
          ...
          prev.set(apiId, this.rpc.d4.getServiceCpnDefaultParams({
            d4StusCd: "1",
            blngtoTyp: "10",
            apiId: apiId,
...
.aop_tradedesign_comps_d5design_process_attr {
  position: relative;
  flex: 0 0 700px;
