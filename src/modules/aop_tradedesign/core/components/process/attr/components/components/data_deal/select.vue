<template>
  <div class="aop_tradedesign_comps_design_attr_data_deal_attr_select">
    <PageDialog
      class="dialog"
      :dialogWidth="'56.3%'"
...
export default {
  ...
  methods: {
    showDialog(currentObj, type) {
      ...
      console.log("showDialog", currentObj, type);
      // this.currentRow = row
      this.type = type;
    ...
    confirm() {
      this.$refs.form.validate(async (valid) => {
        if (!valid) return;
        // const list = this.paramList.filter((item) => item.paramKey == this.formData.paramKey[0])
        // this.currentRow.paramKey = this.formData.paramKey[0]
        ...
        this.$emit("selectConfirm",this.formData,this.tableData,this.paramList,this.type,this.defaultActive)
        this.resetForm();
        this.dialogVisible = false;
      });
    },
    ...
    paramListCellStyle({ row, column, rowIndex, columnIndex }) {
      if (row.paramParentId !== "" && column.type === "selection") {
...
.aop_tradedesign_comps_design_attr_data_deal_attr_select {
  .dialog {
    .tabs_box {
