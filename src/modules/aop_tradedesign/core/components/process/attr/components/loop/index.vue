<template>
  <div class="aop_tradedesign_comps_design_attr_loop_attr">
    <div class="attr_main">
        ...
        <div class="attr_show">
          ...
          <div class="attr_show_item">
            ...
            <div class="attr_show_item_c right">
              <el-select :disabled="!canSave" v-model="formData.loopList" size="mini" placeholder="请选择循环集合">
                <el-option
...
export default {
  ...
  computed: {
    ...
    loopListOptions() {
      ...
      const paramDirection = option?.value == this.transId ? "0" : "1"
        ...
        return [
          {
            ...
            selectVal: () => {
              return this.getDataList().reduce((prev, data) => {
                ...
                return prev
              }, [{ type: "当前4b交易", value: this.transId }])
            },
            clearable: "false"
          },
          {
  ...
  methods: {
    async getParams(e, obj) {
      ...
      if (!e.compositeId) {
        ...
        if (this.formData.loopListSource == "") this.formData.loopListSource = this.transId
    ...
    async saveParams() {
      ...
      const _formData = JSON.parse(JSON.stringify(this.formData))
      _formData.loopListSource = _formData.loopListSource == this.transId ? "" : _formData.loopListSource
...
.aop_tradedesign_comps_design_attr_loop_attr {
  position: relative;
  height: 100%;
  .mt10 {
    margin-top: 10px;
