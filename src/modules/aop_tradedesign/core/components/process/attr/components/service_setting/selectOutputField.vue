...
export default {
  ...
  methods: {
    async showDialog(data, formData) {
      ...
      // if (data.actionId == this.transId || data.actionNm == "当前4b交易") {
      ...
      const paramDirection = data.actionId == this.transId ? "0" : "1"
      if (!["cacheSessionAction", "getSessionAction"].includes(item.actionType)) {
    ...
    tableCellStyle({ row, column, rowIndex, columnIndex }) {
      if(column.type === "selection") {
        ...
        if (item && ["cacheSessionAction", "getSessionAction", "uploadAction"].includes(item.actionType)) {
