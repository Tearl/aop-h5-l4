<template>
  <div class="aop_tradedesign_comps_design_attr_simulant_select" :style="style">
    <el-select
      v-if="flag"
      ref="elSelectRef"
    ...
    >
      ...
      <div v-else-if="columnLabel == '来源值'" class="aop_tradedesign_comps_design_attr_simulant_select_tree_option_container">
    ...
    options() {
      ...
      if (label == "来源") {
        ...
        return this.getDataList().reduce((prev, data) => {
          ...
          if (item || row.actionId == this.transId) {
            ...
            if (row.actionId == this.currentParentCpnInfo.compositeId) {
              ...
              if (!loopListSource && loopList) {
                // 循环集合来自当前4B
                paramList = JSON.parse(JSON.stringify(this.compositeIdMapToParamList[this.transId]?.paramList || []))
  ...
  methods: {
          ...
          if (this.currentParentCpnInfo?.actionType == "loopAction") {
            ...
            if (loopListSource && !(_item?.paramList?.length)) {
              if (_apiId && ["D4bAction", "D5SelfAction", "D5LinkAction"].includes(_item.actionType)) {
                const _res = await this.rpc.d4.queryL5ParamAndRuleDefinitions({
                  d4StusCd: "1",
                  blngtoTyp: "10",
                  apiId: _apiId,
                  d4bApiId: this.transId,
    ...
    getOptionsName(item) {
      ...
      if (property == "actionId") {
        return item.svcId == this.transId || ["无", "当前循环组件", "固定值"].includes(item.label) ? item.label : `${item.label}(${item.actionSeq})`
      } else {
        return item.label
      }
    },
...
.aop_tradedesign_comps_design_attr_simulant_select {
  position: fixed;
  .select_container {
    width: 0px;
    overflow: hidden;
