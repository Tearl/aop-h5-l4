<template>
  <div class="aop_tradedesign_comps_design_attr_judge_attr_select">
    <PageDialog
      :dialogWidth="'56.3%'"
...
export default {
  ...
  inject: ["getDataList"],
  methods: {
    showDialog(row, currentObj) {
      console.log("showDialog", row, currentObj)
      this.currentRow = row
      this.dataList = this.getDataList();
      this.$nextTick(() => {
        this.cpnList = this.dataList.filter(item => {
          return item.actionId != currentObj.actionId && ["D4bAction", "D5SelfAction", "D5LinkAction", "dataDealAction","newD5Action", "dataListAction"].indexOf(item.actionType) > -1
        })
    ...
    confirm() {
      this.$refs.form.validate(async (valid) => {
        ...
        this.currentRow.actionId = this.formData.actionId
        this.currentRow.actionName = this.formData.actionName,
        this.currentRow.paramCName = `${this.formData.actionName}.${list[0].paramCName}`
        this.resetForm();
        this.dialogVisible = false;
      });
    },
    selectChange(e) {
      ...
      if (target.actionType == "dataDealAction" || target.actionType == "dataListAction") {
    ...
    getDataDealOutputParmas(e) {
      ...
      // this.paramList = reList(e.paramList.filter((item) => item.paramDirection == "1"));
    ...
    async getServiceCpnDefaultParams(e) {
      ...
      // this.paramList = reList(res.paramList.filter((item) => item.paramDirection == "1"));
...
.aop_tradedesign_comps_design_attr_judge_attr_select {
  .edit_form {
    margin-top: 10px;
    @include form;
  }
