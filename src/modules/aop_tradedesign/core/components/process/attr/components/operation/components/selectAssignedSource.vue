<template>
  <div class="aop_tradedesign_comps_design_attr_select_assigned_source">
    <PageDialog
      :dialogTitle="'选择参数'"
      ...
      <div slot="box" class="container">
        ...
        >
          <div class="form_info">
            ...
            <el-form-item :label="isFixedValue ? '固定值:' : '参数:'" prop="paramKey">
              <el-input
                v-if="isFixedValue"
...
export default {
  ...
  computed: {
    assignedSourceOptions() {
      const prev = ["assignedSource1", "assignedSource2"].includes(this.currentKey) ? [
        { label: "当前4b交易", value: this.transId, svcId: this.transId },
    ...
    assignedSourceKeyOptions() {
      ...
      if ((item || compositeId == this.transId) && key) {
        let paramList = this.compositeIdMapToParamList[compositeId]?.paramList || []
        // 被赋值来源 可选择当前4B的输入或者L5的输入
        if (key == "assignedSource0") {
          paramList = paramList.filter(param => param.paramDirection == 0)
        } else {
          paramList = paramList.filter(param => param.paramDirection == 1)
        }
        return paramList.map(param => {
          param.label = param.paramCName
          param.value = this.packageParamKey(param, paramList)
          return param
        }).filter(param => ["int", "byte", "short", "long", "float", "double", "Integer", "Byte", "Short", "Long", "Float", "Double", "BigDecimal"].includes(param.paramType))
      } else {
        return []
      }
    },
  ...
  methods: {
    ...
    async showDialog(key, data, formData) {
      console.log("showDialog", key, data, formData)
      this.currentKey = key
    ...
    confirm() {
      this.$refs.form.validate(async (valid) => {
        ...
        this.$emit("setAssignedSource", this.currentKey, data)
        this.closeDialog()
      });
    },
      ...
      if (type == "actionId") {
        ...
        if (item) {
          ...
          if (!e) {
            ...
            this.formData.apSource = e == this.transId ? "00" : "20"
...
.aop_tradedesign_comps_design_attr_select_assigned_source {
  .container {
    padding: 16px;
  }
  .edit_form {
