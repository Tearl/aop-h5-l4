<template>
  <div class="aop_tradedesign_comps_design_attr_data_deal_attr">
    <div class="attr_main">
      ...
      <div class="attr_box">
        <div class="attr_show underLine">
          <div class="attr_show_item">
            <div class="attr_show_item_t">数据名称：<span v-if="isRead" class="attr_show_item_i">{{formData.actionDesc}}</span></div>
          ...
          <div class="attr_show_item">
            <div class="attr_show_item_t">数据描述：<span v-if="isRead" class="attr_show_item_i">{{formData.actionMsg}}</span></div>
            <div class="attr_show_item_c" v-if="!isRead">
              <el-input
                type="textarea"
    ...
    getNodeList(list,type,data,defaultActive) {
      if(list && list.length > 0) {
        return list.map((item) => {
          item.apSource = defaultActive == "servInput" ? "00" : defaultActive == "servOutput" ? "10" : "20",
          item.paramDirection = type == "s" ? "0" : "1"
          item.paramCName = defaultActive == "cpnOutput" ? `${e.actionName}.${list[0].paramCName}` : list[0].paramCName
          item.paramName = item.paramCName
      ...
      if(tableData.length > 0) {
        if(type == "s") {
          tableData.forEach((item,index) => {
            ...
            this.inputParamsListAtSetting.push({
              apSource: defaultActive == "servInput" ? "00" : defaultActive == "servOutput" ? "10" : "20",
              paramDirection: "0",
...
.aop_tradedesign_comps_design_attr_data_deal_attr {
  position: relative;
  height: 100%;
  .attr_main {
    height: 100%;
