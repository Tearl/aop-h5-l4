<template>
  <div class="aop_tradedesign_comps_design_attr_splice_attr">
    <div class="attr_main">
      ...
      <div v-if="!isRead" class="attr_box">
        <div class="attr_show">
          ...
          <div v-if="formData.format == '字段'" class="attr_show_item">
...
export default {
        ...
        return [
          {
            ...
            minWidth: "40%",
            type: "text",
            filter: scope => this.getSimpleParmaKey(scope.row.paramKey),
            tooltip: true,
          },
          {
            prop: "paramName",
    ...
    outputResult() {
      if (this.formData.format == "JSON") {
        return this.tableData.reduce((prev, item) => {
          const paramKey = this.getSimpleParmaKey(item.paramKey)
  ...
  methods: {
    ...
    async saveParams() {
      ...
      } else if (this.formData.format == "字段" && !this.formData.symbol) {
...
.aop_tradedesign_comps_design_attr_splice_attr {
  position: relative;
  height: 100%;
  .attr_main {
    height: 100%;
