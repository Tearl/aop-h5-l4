<template>
  <div class="aop_tradedesign_comps_design_attr_create_graph_validate_code_attr_select_source">
    <PageDialog
      :dialogTitle="'选择参数'"
...
export default {
  ...
  methods: {
    ...
    async showDialog(key, data) {
      ...
      if (data[key]) this.formData = JSON.parse(data[key])
    ...
    confirm() {
      this.$refs.form.validate(async (valid) => {
        ...
        this.$emit("confirm", this.currentKey, data)
        this.closeDialog()
      });
    },
...
.aop_tradedesign_comps_design_attr_create_graph_validate_code_attr_select_source {
  .container {
    padding: 16px;
  }
  .edit_form {
