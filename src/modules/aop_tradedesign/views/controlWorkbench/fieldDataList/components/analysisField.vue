<template>
  <div class="info-content">
    <div class="drawer-main-content info-form-content">
      ...
      <div class="aop-com-info-form-wrap">
        ...
        <div class="padding-b-12">
          <el-button @click="handleAddWordRoots" class="com-primary-medium-btn" icon="el-icon-plus" type="primary"
            size="small">新增词根</el-button>
        </div>
          ...
          <el-table v-loading="isLoading" :data="tableData" @row-click="singleSelect" highlight-current-row>
            <el-table-column prop="rootIdCombine" label="" align="center" min-width="55">
              <template slot-scope="scope">
                <el-radio v-model="tableSelection" :label="scope.row.rootIdCombine">&nbsp;</el-radio>
              </template>
            </el-table-column>
            <el-table-column prop="rootShortNameCombine" label="推荐名称" min-width="100"></el-table-column>

            <el-table-column v-for="index in maxRootCount" :prop="'rootName_' + (index - 1)" :label="'词根' + index"
              :key="index" min-width="120">
              <template slot-scope="scope">
                <span>{{ scope.row["rootName_" + (index - 1)] }}（{{
                  scope.row["rootFullName_" + (index - 1)] || "缺失"
                }}）</span>
              </template>
            </el-table-column>
          </el-table>
    ...
    <!-- <div class="info-bottom-handle">
      ...
      <el-button class="ml24" type="primary" size="small" @click="handleConfirm">
        下一步
      </el-button>
    </div> -->
    <!-- 新增词根 -->
  ...
  methods: {
          ...
          this.rpc.datadict
            ...
            .then((res) => {
              ...
              infoList.forEach((item, index) => {
                ...
                rootData["rootFullName_" + index] = item.rootFullName;
              ...
              if (rootShortNames.length === infoList.length) {
                ...
                combine.forEach((item, i) => {
                  if (i > 0) {
                    // 拼接时除第一个词外首字母大写
                    combine[i] =
                      combine[i].slice(0, 1).toUpperCase() +
                      combine[i].slice(1);
                  }
                });
                rootData.rootShortNameCombine = combine.join("");
              }
              if (rootFullNames.length === infoList.length) {
                // 全称以空格拼接
                rootData.rootFullNameCombine = rootFullNames.join(" ");
              }
              this.tableData = [
                {
                  ...rootData,
    ...
    handleConfirm() {
      ...
      let selected = this.tableData.find((item) => {
        return this.tableSelection && this.tableSelection == item.rootIdCombine;
      });
