  ...
  <div class="info-content">
    <div v-loading="isLoading" class="drawer-main-content info-form-content">
      ...
      <div class="aop-com-info-form-wrap">
        <!-- 模块信息表单 -->
        <el-form ref="infoForm" :model="formData" :rules="formRules" label-position="top" :inline="false" size="small"
          @submit.native.prevent>
          <el-row :gutter="24">
            <el-col :span="12">
              <el-form-item label="字段名称：" prop="fieldName">
                <el-input class="input-primary-btn" v-model="formData.fieldName" placeholder="请输入字段中文名称"
                  :disabled="action != 'addField'" :maxlength="20">
                </el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="字段简称：" prop="fieldKeyName">
                <el-input v-model="formData.fieldKeyName" placeholder="请输入字段简称" :maxlength="20"
                  :disabled="true"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="字段全称：" prop="fieldFullName">
                <el-input v-model="formData.fieldFullName" placeholder="请输入字段全称" :maxlength="20"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="别名：" prop="fieldAlias">
                <el-input v-model="formData.fieldAlias" placeholder="请输入别名" :maxlength="20"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="24">
              <el-form-item label="字段定义：" prop="fieldBusiness">
                <dataFormatTip type="define" class="label-info" />
                <el-input v-model="formData.fieldBusiness" type="textarea" :rows="3" resize="none" placeholder="请输入字段定义"
            ...
            <el-col :span="24">
              <el-form-item label="字段规则：" prop="fieldRule">
                ...
                <el-input v-model="formData.fieldRule" type="textarea" :rows="3" resize="none" placeholder="请输入字段规则"
                  :maxlength="200"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="主题分类：" prop="standards">
                <el-cascader placeholder="请选择" v-model="formData.standards" :options="preNodeProps" clearable :props="{
                  lazy: true,
                  lazyLoad,
                  checkStrictly: true,
                  emitPath: true,
                }"></el-cascader>
                <el-tooltip class="item" effect="dark" content="点击去分类管理" placement="top-start">
                  <img @click="gotoStandards" width="16px" height="16px"
                    :src="require('@m/assets/images/icon_setting.png')" class="theme-class" alt="" />
                </el-tooltip>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="字段类型：" prop="fieldType">
                <el-select v-model="formData.fieldType" placeholder="请选择字段类型" @change="changeFieldType" clearable>
            ...
            <el-col :span="12">
              <el-form-item label="字段长度：" prop="fieldLength">
                <el-input v-model="formData.fieldLength" placeholder="请输入字段长度" :maxlength="20"></el-input>
              </el-form-item>
            </el-col>

            <el-col :span="12">
              <el-form-item label="数据格式：" prop="dataFormatType" v-if="!!formData.fieldType && mapFormatList.length">
                <el-select v-model="formData.dataFormatType" @change="changeDataFormatType" placeholder="请选择数据格式"
                  clearable>
                  <el-option v-for="(item, i) in mapFormatList" :key="i" :label="item.label"
                    :value="item.value"></el-option>
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item v-if="mapPercent.includes(formData.dataFormatType)" label="数据精度：" prop="fieldScale">
                <el-input v-model="formData.fieldScale" placeholder="请输入数值,用于小数点保留位数" :maxlength="20"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="长度限制：" prop="fieldLengthLimitType"
                v-if="!mapLongLimit.includes(formData.dataFormatType)">
                <el-select v-model="formData.fieldLengthLimitType" placeholder="请选择长度限制" @change="changeLength">
                  <el-option v-for="(item, i) in longList" :key="i" :label="item.label" :value="item.value"></el-option>
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="最小长度：" prop="fieldLengthMin" v-if="
              ...
              ">
                <el-input v-model="formData.fieldLengthMin" placeholder="请输入最小长度" :maxlength="20"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="最大长度：" prop="fieldLengthMax" v-if="
                !mapLongLimit.includes(formData.dataFormatType) &&
                !!formData.fieldLengthLimitType &&
                formData.fieldLengthLimitType != '00' &&
                formData.fieldLengthLimitType != '01'
              ">
                <el-input v-model="formData.fieldLengthMax" placeholder="请输入最大长度，不超过字段长度" :maxlength="20"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="取值范围：" prop="valueRange" v-if="mapValueRange.includes(formData.dataFormatType)">
                <el-input v-model="formData.valueRange" placeholder="请输入取值范围，例[0,99]" :maxlength="20"></el-input>
              </el-form-item>
            </el-col>
            <!-- <el-col :span="12">
            ...
            <el-col :span="24">
              <el-form-item label="代码编码规则：" prop="codeRule">
                <el-input type="textarea" v-model="formData.codeRule" placeholder="请输入代码编码规则"
        ...
        <div class="padding-b-12">
          <el-button @click="handleEnumChose" class="com-plain-primary-btn" icon="el-icon-plus" type="default"
            size="small">选择枚举值</el-button>
          <el-button @click="handleEnumAdd" class="com-plain-primary-btn" icon="el-icon-plus" type="default"
            size="small">新增枚举值</el-button>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      action: "addField", // addField/reviewField/editField
      // 表单数据
      formData: {
        fieldName: "",
        fieldKeyName: "",
  ...
  created() {
    ...
    this.analysisConfirm()
  },
  computed: {
    ...mapState("aop_tradedesign", ["fieldObj"]),
    mapFormatList() {
      return this.mapFormatType[this.formData.fieldType];
    },
  },
  methods: {
    ...mapActions("aop_tradedesign", ["saveFieldObj"]),
    gotoStandards() {
    ...
    async getName(codes) {
      ...
      let topNameList = this.preNodeProps.filter(item => item.value === code1)
      topNameList.length > 0 ? this.formData.topicName = topNameList[0].topicName : this.formData.topicName = ''
      const { categoryInfoList } = await this.rpc.datadict.qryDataDictionaryCategoryList({
        topicCode: code1
      });
      let categoryList = categoryInfoList.filter(item => item.categoryId === code2)
      categoryList.length > 0 ? this.formData.categoryName = categoryList[0].categoryName : this.formData.categoryName = ''
    },
    // 点击取消按钮
    handleClose() {
   ...
   async handleConfirm() {
     ...
     await this.getName(this.formData.standards)
      if (
        !!this.formData.fieldLengthMin &&
      ...
      this.$refs.infoForm.validate((valid, validObject) => {
        if (valid === true) {
          ...
          if (this.action === "reviewField") {
            ...
            //   .addDataDictionaryItemAndResponse({
            //     ...this.formData,
            //     fieldSource: "common",
            //     enumCode: this.enumCode,
            //   })
            //   .then((res) => {
            let parmas = {
              // ...res.dataDictionaryFiled
              ...this.formData,
              fieldSource: "common",
              enumCode: this.enumCode,
              dataDictionaryEnumLists: this.tableData
            }
            this.$emit("refreshData", parmas);
            this.handleClose();
            //   })
            //   .catch(() => { });
            // this.$emit('refreshData', {
