    ...
    <el-dialog class="aop_tradedesign_comps_page_dialog" :visible.sync="dialogVisiable" :width="dialogWidth"
        ...
        <div class="dialog_box">
            ...
            <div class="box_header">
                <div class="box_l box_wrap">
                    <div class="comparison_box_title"><span>申请对象</span></div>
                    <div class="info_card">
                        <div class="info_t">
                    ...
                    <div class="list_box">
                        <template v-if="getProcessedInfo('left', 'attribute').length > 0">
                            <div class="list_item" v-for="(item, index) in getProcessedInfo('left', 'attribute')"
                                ...
                                <span class="desc desc_title">{{ item.dictryName + '(' + item.dictryNo + ')' }}</span>
                                <!-- 匹配状态图标 -->
                    ...
                    <div class="list_box">
                        <template v-if="getProcessedInfo('right', 'attribute').length > 0">
                            <div class="list_item" v-for="(item, index) in getProcessedInfo('right', 'attribute')"
                                :key="`attr-right-${index}`" :data-id="item.obaId"
                                :data-type="`attribute-right-${item.obaId}`"
                                @click="handleRightItemClick('attribute', item.obaId)"
                                :class="{ 'selected': isRightItemSelected('attribute', item.obaId) }">
                                <span class="title">字段名称</span>
                                <span class="desc desc_title" style="flex: 1;">{{ item.dictryName + '(' + item.dictryNo
                                    + ')' }}</span>
                            </div>
                        </template>
                        <noData v-else />
                    ...
                    <div class="list_box">
                        <template v-if="getProcessedInfo('right', 'behavior').length > 0">
                            <div class="list_item" v-for="(item, index) in getProcessedInfo('right', 'behavior')"
                                ...
                                @click="handleRightItemClick('behavior', item.obeId)"
                                :class="{ 'selected': isRightItemSelected('behavior', item.obeId) }">
                                <span class="title">行为名称</span>
                                <span class="desc desc_title" style="flex: 1;">{{ item.obeName + '(' + item.obeNo + ')'
                                    }}</span>
                            </div>
                        </template>
                        <noData v-else />
        ...
        <span slot="footer" class="dialog_footer" v-show="footerShow">
            <slot name="footer">
                ...
                <el-button type="primary" @click.native="handleSave">选中</el-button>
            </slot>
        </span>
    </el-dialog>
</template>
