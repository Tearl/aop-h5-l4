<template>
    <div class="edit_object">
        <el-drawer :title="title" :visible.sync="value" class="workspace_common_drawer" direction="rtl"
                    ...
                    <el-form :model="formData" ref="formDataOne" :rules="rules" label-width="110px">
                        ...
                        <el-form-item label="所属业务域" prop="bizDomainNo">
                            <el-select placeholder="请选择所属业务域" style="width: 100%" v-model="formData.bizDomainNo"
                                ...
                                <el-option v-for="item in bizDomainList" :key="item.bizDomainNo"
                                    :label="item.bizDomainName" :value="item.bizDomainNo"></el-option>
                            </el-select>
                        </el-form-item>
                        <el-form-item label="所属中心" prop="applicationNo">
                            <el-select placeholder="请选择所属中心" style="width: 100%" v-model="formData.applicationNo"
                                ...
                                <el-option v-for="item in appAndBlgClsList" :key="item.projectId"
                                    :label="item.projectNm" :value="item.projectId"></el-option>
                            </el-select>
                        </el-form-item>
            ...
            <div class="demo-drawer__content" v-show="activeName == 'tab2'">
                <div class="body">
                    ...
                    <el-table :data="tableData">
                        ...
                        <el-table-column prop="address" label="操作" min-width="80px" fixed="right"
                            v-if="type != 'object'">
                            <template slot-scope="scope">
                                <div class="button">
                                    <span @click="doOperate1('delete', scope.$index, scope.row)"
                                        class="OperateTextR">删除</span>
                                </div>
                            </template>
                        </el-table-column>
                    </el-table>
            ...
            <div class="demo-drawer__content" v-show="activeName == 'tab3'">
                ...
                <div class="body content">
                    <div class="obj-relation-content">
                        ...
                        <el-table :data="formData.objectRelInfos" :key="itemKey" style="width: 100%; margin-top: 14px"
                            align="center" :stripe="false" :header-cell-style="{
                                background: '#F5F5F8',
                                color: '#333',
                            }">
                            ...
                            <el-table-column label="操作" width="180" fixed="right">
                                <template slot-scope="scope">
                                    ...
                                    <template v-else>
                                        <el-button @click="updateObjRelationFun(scope.$index, scope.row)"
            ...
            <div class="demo-drawer__content" v-show="activeName == 'tab4'">
                <div class="body">
                    <el-button type="primary" size="medium" @click="addBehavior('add')" class="btn"
                        style="margin-bottom: 15px">
                        <i class="el-icon-plus"></i>
                        添加对象行为</el-button>
                    <div class="behavior_box">
                        <div class="list_box" v-if="sideTabList && sideTabList.length > 0">
                            <div v-for="(item, index) in sideTabList" :key="index"
                        ...
                        <div class="content_list" v-if="showForm || sideTabList.length > 0">
                            <el-form ref="behaviorForm" :model="form" label-width="120px" :rules="behaviorRules">
                                ...
                                <el-form-item label="行为名称：" prop="obeName">
                                    <el-input class="input" v-model="form.obeName" @input="inputChange" clearable
                                        placeholder="请输入行为名称">
                                    </el-input>
                                </el-form-item>
                                <el-form-item label="行为编码：" prop="obeNo">
                                    <el-input class="input" v-model="form.obeNo" clearable @input="inputChange"
                                        placeholder="字母、数字组合的编码，小写开头">
                                    </el-input>
                                </el-form-item>
                                <el-form-item label="行为描述：" prop="obeDesc">
                                    <el-input type="textarea" class="input" v-model="form.obeDesc" @input="inputChange"
                            ...
                            <el-row style="display: flex;align-items: center;">
                                <div style="margin-right: 8px;"><span
                                        style="color: #F56C6C; margin-right: 4px;">*</span>选择S6服务
                                </div>
                    ...
                    <!-- <el-table :data="sideTabList">
                        ...
                        <el-table-column prop="address" label="操作" min-width="60px" fixed="right">
                            <template slot-scope="scope">
                                ...
                                <el-button @click="deleteData(scope.$index, scope.row)" type="text"
                                    style="color: #fe4758">删除</el-button>
                            </template>
                        </el-table-column>
                    </el-table> -->
                </div>
            ...
            <div class="demo-drawer__content" v-show="activeName == 'tab5'">
                ...
                <div class="body">
                    <template v-if="formData.objType == '00'">
                        <el-button type="primary" size="small" @click="selectLibraryTableObject"
                            :disabled="showSysBtn">选择库表对象</el-button>
                    </template>
                    <el-table :data="formatSysObjList(data1)" ref="multipleTableTwo">
                        <el-table-column prop="objCd" label="库表对象编码" show-overflow-tooltip
                            min-width="120px"></el-table-column>
                        <el-table-column prop="objNm" label="库表对象名称" show-overflow-tooltip
                            min-width="120px"></el-table-column>
                        <el-table-column prop="bizDomainNm" label="业务域" show-overflow-tooltip
                            min-width="120px"></el-table-column>
                        <el-table-column prop="appCntrNm" label="所属中心" show-overflow-tooltip
                            min-width="120px"></el-table-column>
                        <el-table-column prop="objClsNm" label="分类" show-overflow-tooltip
                            min-width="120px"></el-table-column>
                        <el-table-column prop="attrs" label="属性" show-overflow-tooltip
                            min-width="120px"></el-table-column>
                        <el-table-column label="操作" width="120px" fixed="right">
                            <template slot-scope="scope">
                                ...
                                <template v-if="formData.objType == '00'">
                                    ...
                                    <el-button style="color: #f56c6c" type="text" v-if="!showSysBtn"
                                        @click="deleteData1(scope.row)" size="small">删除</el-button>
                                </template>
                            </template>
                        </el-table-column>
                    </el-table>
        ...
        <ObjectInfo :objId="objectInfoId" v-if="showObjectInfo" :visible="showObjectInfo"
            @close="showObjectInfo = false">
        </ObjectInfo>
        ...
        <AddSysObj v-if="addSysObjVisible" :visible="addSysObjVisible" :project="project"
            @close="addSysObjVisible = false" @select="selectSysObj" />
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        async qryObjBizObjectInfoV2() {
            ...
            let res = JSON.parse(JSON.stringify(this.basicInfo));
            console.log("res:", res);
            ...
            this.queryAppAndBlgClsList()
            let data = await this.rpc.controlWorkbench.queryAppAndBlgClsList({
                bizDomainNo: this.formData.bizDomainNo,
            })
            this.appAndBlgClsList = data.queryAppAndBlgClsInfoList || []
        ...
        async updateObjRelationFun(index, data) {
            this.$refs.formDataOne.validate(async (valid) => {
                if (valid) {
                    await this.queryAllObj();
                    let all = this.objList.filter(
