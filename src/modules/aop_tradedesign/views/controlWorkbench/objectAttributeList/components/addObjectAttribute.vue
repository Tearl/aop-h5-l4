    ...
    <div style="width: 100%; height: 100%">
        <div class="add_obj_behavior_box">
            ...
            <div class="base_info_box" id="applyDetailBox">
                ...
                <div class="card_desc">新增对象属性申请</div>
                <div class="info_row1">
                ...
                <div class="tab_content">
                    <objectPage :activeTabs="activeName" :basicInfo="basicInfo" :editAttr="true"></objectPage>
                </div>
            </div>
                ...
                <div class="table_box">
                    <el-table :data="applySourceList" border>
                        ...
                        <el-table-column label="操作" width="120px" fixed="right">
                            <template slot-scope="scope">
                                <el-button type="text" size="medium" @click="handleClick(scope.row, 'lookAttr')">
                                    查看
                                </el-button>
                            </template>
                        </el-table-column>
            ...
            <div class="base_info_box" id="applyHandleBox">
                ...
                <div class="table_box approve_table_box">
                    <!-- confim 抛出修改的对象 -->
                    <editObject ref="editObject" :activeName="'tab2'" :objId="editId" @confim="confim"
                        :addAttr="true" />
                </div>
            </div>
...
export default {
    ...
    computed: {
        selectedItem() {
            ...
            let item = this.tableData.find((v) => v.objId == this.rowIdKey);
            return item;
        },
        selectedNum() {
    ...
    created() {
        const { aplyTargetId, aplySrlNo } = this.$route.query;
        this.editId = aplyTargetId;
        this.aplySrlNo = aplySrlNo;
        Promise.all([
            this.qryBizObjectInfoV2(aplyTargetId),
    ...
    methods: {
        ...
        async getList() {
            let res = await this.rpc.controlWorkbench.qryBizD1ObjectListV2({
                ...
                // isExact: this.searchForm.isExact
            });
        ...
        async qryBizObjectInfoV2(rowId) {
            ...
            this.basicInfo.attributeInfos = data.infoList || [];
        ...
        onConfirm() {
            ...
            // }
            let approveNum = this.$refs.editObject.getIsApprovedNum();
            if (approveNum > 0) {
                this.$message.error("请先审批未审批对象属性");
                return;
            }
            let num = this.$refs.editObject.getIsVerifiedNum();
            if (num > 0) {
                this.$message.error("请先核实未核实对象属性");
                return;
            }
            let tableData = this.$refs.editObject.getTableData();
            let tableDataAttr = this.$refs.editObject.getTableDataAttr();
            let attrArr = JSON.parse(JSON.stringify(this.basicInfo));
            attrArr.attributeInfos = tableData || [];
            this.basicInfo1 = JSON.parse(JSON.stringify(attrArr));
            this.dialogBussinessVisiable = true;
            // let parmas = {
            //     aplySrlNo: this.detailData.aplySrlNo,
            //     aplyTargetId: this.detailData.aplyTargetId,
            //     aplyTargetType: "03",// 00-对象;01-对象行为,02-字段,03-对象
            //     fieldList: tableData || [],
            //     fieldListNew: tableDataAttr || [],
            // }
            // this.rpc.controlWorkbench.submitApproval({
            //     ...parmas
            // }).then(res => {
        ...
        saveForm(val) {
            let tableData = this.$refs.editObject.getTableData();
            let tableDataAttr = this.$refs.editObject.getTableDataAttr();
            // console.log(tableData, tableDataAttr);
            let parmas = {
                aplySrlNo: this.detailData.aplySrlNo,
                aplyTargetId: this.detailData.aplyTargetId,
                aplyTargetType: "03",// 00-对象;01-对象行为,02-字段,03-对象
        ...
        sumbitFn(val) {
            ...
            this.rowIdKey = this.selectItem.objId;
            // this.selectItem = JSON.parse(JSON.stringify(val));
            this.dialogVisiable = false;
            this.addKeyActVisible = false;
        },
