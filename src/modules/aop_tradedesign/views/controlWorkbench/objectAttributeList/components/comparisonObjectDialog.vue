<template>
    <el-dialog class="aop_tradedesign_comps_page_dialog" :visible.sync="dialogVisiable" :width="dialogWidth"
        :modal-append-to-body="modalAppendToBody" :before-close="handleClose" :append-to-body="apTobody"
        :close-on-click-modal="false" :custom-class="customClass" center>
        <template slot="title">
        ...
        <div class="dialog_box">
            ...
            <div class="box_header">
                <div class="box_l box_wrap">
                    <div class="comparison_box_title"><span>对象属性申请</span></div>
                    <div class="info_card">
                        <div class="info_t">
                            <div class="left">{{ basicInfo.objName || '/' }}</div>
                            <div class="right">{{ basicInfo.objType == "00" ? "实体对象" : "值对象" || '/' }}</div>
                        </div>
                        <div class="info_t">
                            <div class="left_b">申请人：{{ detailData.aplyUserName || '/' }}</div>
                            <div class="right_b">申请时间: {{ detailData.aplyTime || '/' }}</div>
                        </div>
                    </div>
                </div>
                <div class="box_r box_wrap">
                    ...
                    <div class="info_card">
                        <div class="info_t">
                            <div class="left">{{ selectedItem.objName || '/' }}</div>
                            <div class="right">{{ selectedItem.objType == "00" ? "实体对象" : "值对象" || '/' }}</div>
                        </div>
                        <div class="info_t">
                            <div class="left_b">创建人：{{ selectedItem.createUser || '/' }}</div>
                            <div class="right_b">创建时间：{{ selectedItem.createTime || '/' }}</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="box_center">
                ...
                <div class="card_title card_title_wrap box_wrap">
                    <div class="title_wrap">
                        <span class="d_line"></span>
                        <span class="d_detail">对象属性</span>
                        <span class="tips" v-if="!isAllMatch('attribute')">属性不满足</span>
                        <span class="tips_success" v-if="isAllMatch('attribute')">属性满足</span>
                    </div>
            ...
            <div class="box_bottom">
                ...
                <div class="table-left box_wrap_b">
                    <div class="list_box">
                        <template v-if="getProcessedInfo('left', 'attribute').length > 0">
                            <div class="list_item" v-for="(item, index) in getProcessedInfo('left', 'attribute')"
                                :key="`attr-left-${index}`" :data-id="item.obaId"
                                :data-type="`attribute-left-${item.obaId}`" :class="{
                                    'selected': selectedLeftItem.type === 'attribute' && selectedLeftItem.id === item.obaId,
                                    'mapped': isItemMatch(item, 'attribute')
                                }">
                                <span class="title">字段名称</span>
                                <span class="desc desc_title" style="flex: 1;" :title="item.fieldName + '(' + item.fieldId + ')'">{{
                                    item.fieldName + '(' + item.fieldId + ')' }}</span>
                                <!-- 匹配状态图标 -->
                                <span class="img" v-if="!isItemMatch(item, 'attribute')">
                                    <img src="../../assets/images/false.png" alt="未匹配">
                                </span>
                                <span class="img" v-if="isItemMatch(item, 'attribute')">
                                    <img src="../../assets/images/success.png" alt="已匹配">
                                </span>
                            </div>
                        </template>
                        <noData v-else />
                ...
                <!-- 中间连线区域（SVG容器） -->
                <div class="table-center" ref="attributeConnector" data-type="attribute">
                    <!-- SVG动态生成连线 -->
                ...
                <div class="table-right box_wrap_b">
                    <div class="list_box">
                        <template v-if="getProcessedInfo('right', 'attribute').length > 0">
                            <div class="list_item" v-for="(item, index) in getProcessedInfo('right', 'attribute')"
                                :key="`attr-right-${index}`" :data-id="item.oldObaId"
                                :data-type="`attribute-right-${item.oldObaId}`"
                                :class="{ 'selected': isRightItemSelected('attribute', item.oldObaId) }">
                                <span class="title">字段名称</span>
                                <span class="desc desc_title"  :title="(item.useFieldName || item.fieldName) + '(' + (item.directNo || item.fieldId) + ')'">{{
                                    (item.useFieldName || item.fieldName) + '(' + (item.directNo || item.fieldId) + ')' }}</span>
                                <!-- <span class="desc desc_title"
                                    :title="item.useFieldName || `${item.fieldName}(${item.directNo || item.fieldId})`">
                                    {{ item.useFieldName || `${item.fieldName}(${item.directNo || item.fieldId})` }}
                                </span> -->
                            </div>
                        </template>
                        <noData v-else />
        ...
        <span slot="footer" class="dialog_footer" v-show="footerShow">
            <slot name="footer">
                ...
                <el-button type="primary" :disabled="!isAllMatch('attribute')"
                    @click.native="handleSave">确认提交</el-button>
            </slot>
        </span>
    </el-dialog>
</template>
...
export default {
    ...
    mounted() {
        this.$nextTick(() => {
            ...
            this.getRoleFn()
            this.$forceUpdate();
            console.log(this.selectedItem, 'this.selectedLeftItem');
        });
    ...
    methods: {
        ...
        handleLeftItemClick(type, leftId) {
            if (this.selectedLeftItem.type === type && this.selectedLeftItem.id === leftId) {
                this.selectedLeftItem = { type: null, id: null };
                return;
            }
        ...
        handleRightItemClick(type, rightId) {
            ...
            Object.entries(this.manualMap[type]).forEach(([leftId, mappedRightIds]) => {
                if (leftId !== currentLeftId && mappedRightIds.includes(rightId)) {
                    isRightUsed = true;
                }
            });
            if (isRightUsed) {
                this.$message.warning("该右侧项已被其他左侧项映射，无法重复关联");
                return;
            }

            // 4. 未被占用：正常处理映射（原有逻辑不变）
            if (this.manualMap[type][currentLeftId]?.includes(rightId)) {
                // 已有映射：取消当前左侧与该右侧的关联
                this.manualMap[type][currentLeftId] = this.manualMap[type][currentLeftId].filter(id => id !== rightId);
                if (this.manualMap[type][currentLeftId].length === 0) {
                    delete this.manualMap[type][currentLeftId];
                }
            } else {
                if (!this.manualMap[type][currentLeftId]) {
                    this.manualMap[type][currentLeftId] = [];
                }
                this.manualMap[type][currentLeftId].push(rightId);
            }
            this.clearLines(type);
            this.drawAllLines(type);
            this.selectedLeftItem = { type: null, id: null }; // 取消左侧选中状态
        },
        isRightItemSelected(type, rightId) {
            if (!this.selectedLeftItem.type || this.selectedLeftItem.type !== type) return false;
            const leftId = this.selectedLeftItem.id;
        ...
        initDefaultMap(type) {
            const { leftKey, rightKey, leftIdKey, rightIdKey } = this.infoConfig[type];
            // 从最新的props数据中获取左侧/右侧列表（避免使用旧数据）
            ...
            leftData.forEach(leftItem => {
                ...
                // 匹配逻辑：仅基于当前存在的右侧item（已删除的右侧item不参与匹配）
        ...
        drawAllLines(type) {
            ...
            const svg = this.$refs[`${type}Svg`];
            if (!svg) return;
            Object.entries(this.manualMap[type]).forEach(([leftId, rightIds]) => {
                rightIds.forEach(rightId => {
                    // 找到对应的左右项数据
                    const leftItem = leftData.find(item => item[leftIdKey] === leftId);
                    const rightItem = rightData.find(item => item[rightIdKey] === rightId);
        ...
        drawSingleLine(type, leftId, rightId) {
            // 获取左右项的DOM元素
            const leftEl = document.querySelector(`[data-type="${type}-left-${leftId}"]`);
            const rightEl = document.querySelector(`[data-type="${type}-right-${rightId}"]`);
            const svg = this.$refs[`${type}Svg`];
            ...
            const startY = leftRect.top + leftRect.height / 2 - connectorRect.top;
            const endX = rightRect.left - connectorRect.left;
            const endY = rightRect.top + rightRect.height / 2 - connectorRect.top;
            ...
            line.setAttribute('data-connection', `${type}-${leftId}-${rightId}`);
            line.addEventListener('click', (e) => {
                e.stopPropagation();
                if (this.manualMap[type][leftId]) {
                    this.manualMap[type][leftId] = this.manualMap[type][leftId].filter(id => id !== rightId);
        ...
        //     const leftData = this.basicInfo[this.infoConfig[type].leftKey] || [];
        //     const rightData = this.selectedItem[this.infoConfig[type].rightKey] || [];
        //     if (side === 'left') {
        //         return leftData;
        //     }
        ...
        //         return [...rightData].sort((a, b) => a[rightIdKey] - b[rightIdKey]);
        //     }

        //     // 基于最新的初始映射状态筛选（避免残留已删除item的映射）
        //     const { rightIdKey } = this.infoConfig[type];
        //     const mappedItems = rightData
        //         .filter(item => this.initialMappedRightIds[type].has(item[rightIdKey]))
        ...
        handleSave() {
            ...
            //     mappings: this.manualMap  // 传递映射关系
            // }
        },
