<template>
    <div class="d1_app_design_page_new">
        <div class="right_main" v-if="aaiStatus == '01'">
            ...
            <div class="appInfo_operate_block">
                ...
                <div class="appInfo_operate_block_right" v-if="action != 'app-look'">
                    <el-popover width="100px" trigger="hover" placement="bottom" popper-class="d1Design_operate_more">
                        <div class="operate_more_btn">
                            <span @click="doOperate('check-pass')">验收通过</span>
                            <span>历史版本选择</span>
                        </div>
                        <el-button  slot="reference" size="small" type="primary">更多操作</el-button>
                    </el-popover>
                </div>
            </div>
...
<script>
import d1FuncList from './components/funcList.vue'
import streamMatrix from '../components/streamMatrix.vue'
import dataSet from './components/dataSet_new.vue'
import serviceSet from './components/serviceSet_new.vue'
...
export default {
    ...
    methods: {
        ...
        async queryAppInfos() {
            const { aaiStatus, aaiName, aaiVersion } = await this.rpc.d1workspace.d1AppInfo({
        ...
        doOperate(operate) {
            switch(operate) {
                case "app-info":
                    ...
                    break;
                case 'check-pass':
                    this.$confirm("确定验收通过当前应用吗？", "温馨提示").then(
                        (res) => {
                            this.rpc.d1workspace.d1AppSubmitCheck({
                                aaiId: this.appInfo.aaiId,
                                aaiVersion: this.appInfo.aaiVersion,
                                isolateWorkspace: this.appInfo.isolateWorkspace
                            }).then(res => {
                                ...
                                this.$router.replace({path:'/aop_enddesign/d1interation/detail',query:{...this.$route.query,d1AppIndex:'2'}})
                            })
                        }
                    );
                    break
