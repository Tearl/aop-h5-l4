<template>
    <el-dialog
        title="导入历史应用"
        :append-to-body="true"
        class="inner_cheetah_common_dialog inner_cheetah_comps_historyVersionChoseDialog"
    ...
    >
        <div class="flex">
            ...
            <div class="right_main" v-if="treeData[0].children.length > 0">
                ...
                <component ref="matrix" v-bind:is="currentTabComponent" :appInfos="appInfos"></component>
            </div>
...
<script>
import lookd1ModuleFuncList from '../moduleInfo/moduleFuncList.vue'
import lookmoduleStreamMatrix from '../appInfo/historyMatrix.vue'
    ...
    methods: {
        submitForm() {
            if(JSON.stringify(this.checkedData) === "{}") {
                ...
                this.rpc.d1workspace.belongMatrix({
                    isolateWorkspace: this.appInfo.isolateWorkspace,
                    belongId: this.appInfo.aaiId,
                    belongVersion: this.appInfo.aaiVersion,
                    dvmType: '00',
                    dvmUrl: this.$refs.matrix.formData.dvmUrl.join(','),
                    dvmId: this.$refs.matrix.formData.dvmId
                }).then(res => {
                    this.$emit('output')
        ...
        async initData() {
            const { appInfoLists } = await this.rpc.d1workspace.appHistoryVersion({
                aaiId: this.appInfo.aaiId
            })
            
            let formatList = appInfoLists.map((item, index) => {
                return {
                    label: item.aaiVersion,
                    id: `0.${index}`,
                    ...item
                }
            })
            ...
            this.treeData[0].label = '应用版本列表'
            this.treeData[0].disabled = true
            if(formatList.length == 0) {
                this.showCheckbox = false
            } else {
                const { aaiId, aaiVersion } = formatList[0]
                this.appInfos = {
                    aaiId,
                    aaiVersion
                }
...
.inner_cheetah_comps_historyVersionChoseDialog {
    /deep/.el-dialog {
        background: #F5F5F8;
    }
