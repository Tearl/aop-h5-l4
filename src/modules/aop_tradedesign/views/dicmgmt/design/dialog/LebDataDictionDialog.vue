        ...
        <div class="leb_filter">
            <el-form ref="form" :model="formData" :inline="true" class="leb_filter_form" label-width="90px"> 
                <div>
                    ...
                    <!-- <el-form-item class="leb_formitem" label="" prop="kind">
                        <el-select v-model="formData.kind" placeholder="所属分类" :clearable="true"/>
                    </el-form-item> -->
        ...
        <div class="leb_body">
            <el-table :data="iDictList" class="leb_common_table" row-key="id"
                ...
                <el-table-column min-width="8%">
                    ...
                    <el-checkbox :value="bAllSelect" @change="allCheck(scope.column, $event)">&nbsp;</el-checkbox>
                    <span>{{scope.column.label}}</span>
                </template>
                    <template slot-scope="scope">
                        <el-checkbox
                            v-if="!scope.row.parent"
                            :value="!!selectDictMap[scope.row.key]"
                            :disabled="disabledMap[scope.row.key]"
                            @change="changeCheck(scope.row, $event)"><span></span></el-checkbox>
                    </template>
                </el-table-column>
...
export default {
    ...
    methods: {
        ...
        queryDictList (type) {
            ...
            this.rpc.d1workspace.queryDataDictionary({
                content: this.formData.name,
                currentPage: this.currentPage,
                paraGateg: this.formData.kind,
                paraType: this.formData.type,
                preciseQuery: "0",
                turnPageShowNum: this.pageSize
            }).then(res => {
                // 转义接口数据与编辑器约定数据结构
                let list = this.resolveDictList(res.dictionariesInfoBoList, null)
                this.iDictList = list || []
        ...
        resolveDictList(list = [], parent) {
            return list.map(dictInfo => {
                let dict = {
                    id: dictInfo.dictryId,
                    parent: parent,
                    key: dictInfo.dictryNo,
                    name: dictInfo.dictryNm,
                    type: dictInfo.dictryTyp,
                    desc: dictInfo.dictryDescr,
                    dictryEnum: dictInfo.dictryEnum,
                    // length: dictInfo.dictryLength,
                    children: []
                }
         ...
         */
        submitForm () {
            let resolveDictList1 = (list = []) => {
                return list.map(info => {
                    return {
                        id: info.id,
                        key: info.key,
                        name: info.name,
                        dictryEnum: info.dictryEnum,
                        type: this.typeMaps[info.type] || this.defaultType,
                        defaultValue: '',
                        children: resolveDictList1(info.children || [])
                    }
                })
            }
            let model = {
                list: resolveDictList1(this.iSelectList)
            }
            console.log(model, 'model-----0')
            this.dialogVisible = false
            this.$emit('output', model)
         ...
         * 全选
         */
        allCheck (column, bCheck) {
            this.iDictList.forEach(row => {
                if(!this.disabledMap[row.key]){
                    this.changeCheck(row, bCheck)
                }
            })
        },
