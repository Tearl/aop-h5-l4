<template>
  <div class="aop_tradedesign_comps_workbench_public_dialog_addService">
    ...
    <DataSetList ref="dataSetList" @confirmDataset="confirmDataset">
    </DataSetList>
    <PageDialog :apTobody="true" :dialogTitle="result" :closeDialog="closeDialog" :dialogVisiable="dialogVisiable" dialogWidth="1000px">
      <div slot="box">
        <Table
          slot="box"
          class="table"
    ...
    <div class="content-footer">
      ...
      <el-button type="primary" @click.native="confirm">保存</el-button>
    </div>
...
import { getSessionStorage, setLocalStorage } from "@m/utils/localStorage.js";
...
export default {
  ...
  methods: {
    ...
    async confirmDialog() {
      ...
      if (!this.selectData || !this.selectData.length) {
        this.$message.error("请选择添加字段");
        return
      }
      this.selectData.forEach((item) => {
        if (!item.dataSubstnNo) {
          status = true
        }
      })
      if (status) {
        this.$message.error("请输入选中的所属分段");
        return
      }
      console.log(this.serviceData, "dddd")
      // addDataField
      this.selectData.forEach((item) => {
        item.dataSetNo = this.dataSetNo
        item.paramNm = item.paramName
        item.paramTyp = item.paramType
        item.paramEnum = item.apEnum
        item.l5SvcTyp = this.serviceData.apiServiceType
        this.tableColumn[0]?.selectVal.forEach((data) => {
          if (data.value === item.dataSubstnNo) {
            item.dataSubstnNm = data.type
          }
        })
      })
      console.log(this.selectData, "selectData")
      const params = {
        d5UseDataSetRelList: this.selectData,
      }
      const res = await this.rpc.dicmgmt.addDataField(params)
      this.$notify({
        title: "成功",
        message: "成功",
        duration: 2000,
    ...
    async confirmDataset(data) {
      this.dataSetNo = data.dataSetNo
      this.dataSetNm = data.dataSetNm
      const params = {
        dataSetNo: data.dataSetNo,
        apiId: this.apiId,
      }
      const res = await this.rpc.dicmgmt.checkService(params)
      this.outputParamsListAtSetting= res.puApiParams
      ...
      if (!this.outputParamsListAtSetting.length) {
        this.serviceData.dataSetNo = data.dataSetNo
        this.serviceData.dataSetNm = data.dataSetNm
        this.serviceData.beltLine = data.beltLine
        this.serviceData = JSON.parse(JSON.stringify(this.serviceData))
        return
      }
      const params2 = {
        dataSetNo: data.dataSetNo,
      ...
      const res2 = await this.rpc.dicmgmt.getSubStnList(params2)
      this.tableColumn[0].selectVal = res2.dataSubsectionList.map((item) => {
        item.type = item.dataSubstnNm
        item.value = item.dataSubstnNo
        return item
      })
    ...
    openDataset() {
      ...
      this.$refs.dataSetList.showDialog()
    },
    init() {
      this.$nextTick(() =>{
    ...
    labelClick(e) {
      if (e == "list") {
        ...
        this.$router.push({
          ...
          query: {
            dataSetNo: this.serviceData.dataSetNo,
    ...
    async getApiDetail(data){
      ...
      this.serviceData.beltLine = res.beltLine
      // console.log(res,"--------");
      let obj ={}
      ...
      obj ={
        ...
        svcNm:res.apiName,
        svcNo:res.apiCode,
        dataSetNm:this.serviceData.dataSetNm || this.dataSetNm,
        svcDesc:res.apiDesc,
    ...
    async confirm() {
      ...
      const valid = await this.$refs.basicRef.$refs.formRef.validate()
      ...
      if (form.svcTyp == "D5自有服务") {
        ...
        let noSameList = [];  //拿到新增完成和查询回来的不同参数
        let addParamsList = []; //从noSameList拿到从数据集新增的字段参数
        let sameList = [];   //新增完成和关联表找出相同的数据集字段
        ...
        const params = {
          dataSetNo:this.serviceData.dataSetNo,
          dataSetNm:this.serviceData.dataSetNm,
          apiName:form.svcNm,
          apiDesc:form.svcDesc,
          apiServiceType:form.svcTyp || this.serviceData.apiServiceType,
  ...
  created() {
    this.serviceData = JSON.parse(getSessionStorage('dataObj'))
    console.log(this.serviceData, "serviceData")
    this.apiId = this.serviceData.apiId
    this.init();
  },
};
</script>
...
.aop_tradedesign_comps_workbench_public_dialog_addService {
  // ::v-deep .aop_tradedesign_dic_cmps_data_set_form_serv_Info {
  //   padding: 0px;
  // }
