<template>
  <div class="aop_tradedesign_page_dicmgmt_dic_example_list">
        ...
        >
          <div class="form_info">
            <el-form-item label="示例值" prop="exampleValue">
              ...
              <el-button class="button" type="primary" @click="getExampleValue">随机生成</el-button>
            </el-form-item>
          </div>
        </el-form>
      </div>
...
export default {
  ...
  methods: {
    async getExampleValue() {
      const api = this.sysId ? "getDictionaryExampleByVerifyRule" : "getPublicDictionaryExampleByVerifyRule"
      const { exmpVal } = await this.rpc.dicmgmt[api]({
    ...
    operate(operation, data) {
      if (operation == "delete") {
        this.$confirm("是否删除该示例").then(async () => {
          ...
          const res = await this.rpc.systemmgmt.delDictExample(params);
          this.$notify({
            title: "成功",
            message: "删除成功",
            duration: 2000,
    ...
    async sortDictExample(e) {
      ...
      const res = await this.rpc.systemmgmt.sortDictExample({
        exampleOrderList,
      });
      this.getList();
    },
    ...
    // 添加弹框确定
    confirm() {
      if (!this.formData.exampleValue || !this.formData.exampleValue.trim()) {
        this.$message({
          showClose: true,
          message: '请输入示例值',
          type: 'error'
...
.aop_tradedesign_page_dicmgmt_dic_example_list {
  .service_content {
    padding: 20px 24px 0;
  }
