      ...
      >
        ...
        >
          <el-table-column
            ...
            v-if="selectedKey.includes('dataSetNo')"
          ></el-table-column>
          <el-table-column
            label="数据集名称"
            prop="dataSetNm"
          ...
          <el-table-column
            ...
            prop="classifyId"
            show-overflow-tooltip
            :formatter="classifyIdSt"
          ></el-table-column>
          <el-table-column label="数据集状态" prop="statusCd">
            <template slot-scope="scope">
              <span
          ...
          <el-table-column
            ...
            prop="crtTm"
            width="180"
            :formatter="dateTimeFilter"
            v-if="selectedKey.includes('gmtCreate')"
          ></el-table-column>
            ...
            <template slot-scope="scope">
              <div class="operate">
                ...
                <span
                  ...
                  >设计</span
                >
                <!-- <span class="blue_span" @click="doOperate('reset-design', scope.row)" v-if="scope.row.aaiStatus == '005'">重新设计</span>
                                <span class="blue_span" v-if="scope.row.aaiStatus == '005'" @click="m3NotPassDialogVisible = true">查看原因</span> -->
                <span
                  v-if="scope.row.statusCd == '0'"
                  class="blue_span border_d4_wark_space"
        ...
        <div v-else>
          ...
          <div class="application_manage_list_card" v-else>
            ...
            >
              ...
              <span :class="['status_span', `status_${item.statusCd}`]">{{
                publishStatus[item.statusCd]
              }}</span>
            </div>
          </div>
        </div>
...
import { setSessionStorage, getSessionStorage } from "@m/utils/localStorage.js";
export default {
  mixins: [mixin],
  ...
  data() {
    return {
      ...
      selectedKey: ["dataSetNo"],
      selectItemKeyList: ["dataSetNo"],
      toggleIndex: 0,
      // 待选项的数据列表
      tableColumnList: [
        {
  ...
  mounted() {
    if(typeof JSON.parse(getSessionStorage("changeView")) == "object") this.changeView = true
    else this.changeView = JSON.parse(getSessionStorage("changeView"))
    console.log(typeof JSON.parse(getSessionStorage("changeView")),this.changeView,"---===---")
  },
  methods: {
    init() {
      // this.getDicList();
    ...
    async getDicList(e) {
      let status = [];
      let registerCount = "";
      console.log(this.form.status,this.active,"--------")
      if(this.active == "all"){
        if(this.form.status == "0") status = ["0"]
        else if (this.form.status =="20") status = ["1","2"]
        else if (this.form.status == "01") status = ["3"]
        else status = []
      ...
      const res = await this.rpc.dicmgmt.getDataSetList(queryParams);
      this.list = res.dataSetList || [];
      ...
      switch (operate) {
        ...
        case "design":
          this.$router.push({
            ...
            query: {
              dataSetNo: scope.dataSetNo,
              beltLine: scope.beltLine
            },
          });
          sessionStorage.setItem("datasetBeltLine",scope.beltLine)
          break;
        case "commit":
          this.$confirm("确定要提交评审吗?").then(async () => {
            ...
            const res = await this.rpc.dicmgmt.dataSetSubmitForApproval(params);
            this.$notify({
              title: "成功",
              message: "提交评审成功",
              duration: 2000,
        ...
        case "review":
          ...
          this.$confirm("确定要评审通过吗?").then(async () => {
            const params = {
              dataSetNo: scope.dataSetNo,
              aprvlRsltCd:"0"
            };
        ...
        case "recall":
          this.$confirm("确定要撤回吗?").then(async () => {
            ...
            const res = await this.rpc.dicmgmt.dataSetApprovalCancel(params);
            this.$notify({
              title: "成功",
              message: "撤回成功",
              duration: 2000,
    ...
    btnClick(val) {
      switch (val) {
        ...
        case 1:
          ...
          setSessionStorage("changeView",this.changeView)
          break;
        case 2:
    ...
    changeComlunTable(val) {
      ...
      console.log("1222",this.selectItemKeyList)
    },
