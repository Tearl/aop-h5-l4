          ...
          <div class="input_wrapper">
            <div class="top_right_create">
              ...
              <el-tag size="small" type="warning"
                >1、自定义添加参数时，参数key以英文开头、或数字结尾，参数描述需输入自定义原因
...
import PageBigData from "@m/core/components/page_dic_dataset";
import SelectParam from "../dialog/selcParam";
import AddUserDesign from "../dialog/addUserDesign";

import BodyDefine from "../dialog/addUserDesign"; 
...
export default {
  ...
  methods: {
    ...
    showParams(e) {
      ...
      if (e == "userDefined") this.$refs.userDefinedRef.showDialog(e);
      else this.$refs.dataSetRef.showDialog();
    },
    //大数据的添加
    addBdata(e) {
      this.operate("add", e);
    ...
    operate(operation, data) {
      ...
      if (data.paramDirection == "0") {
        if (operation == "add") {
          if (
            data.paramValueType &&
            ["List", "Object"].includes(data.paramValueType) &&
            data.paramOrder.split(".").length >= 6
          )
            return this.$message.error("数据结构层级只能第五层");
          ...
          // let bodyDataList = this.sendData.filter((item) =>item.paramPosition == "0" &&!item.paramParentId &&item.paramKey == "bodyData")
          // console.log(this.sendData[1],tempList,"bodyDataList")
          // dicSetParamOrder(bodyDataList[0].children,bodyDataList[0].paramOrder)
          // dicSetParamOrder(tempList,tempList[0].paramOrder)
          // this.getNodeId(this.sendData[2].children,this.sendData[2].paramOrder);
          dicSetParamOrder(this.sendData);
        ...
        if (operation == "add") {
          ...
          dicSetParamOrder(this.receiveData);
          console.log(this.receiveData, "receiveData");
          // let bodyOutDataList = this.receiveData.filter((item) =>item.paramPosition == "0" &&!item.paramParentId &&item.paramKey == "bodyOutData")
          // dicSetParamOrder(bodyOutDataList[0].children,bodyOutDataList[0].paramOrder)
    ...
    delParams(arrs, id, data) {
      ...
      // console.log(this.sendData, "111", arrs, "222");
    },
    getSort(arrs, id) {
      let i = 0;
      while (arrs[i]) {
    ...
    saveParams(e, substnData, type) {
      ...
      console.log("data", e, substnData, flag, type);
      const data = JSON.parse(JSON.stringify(e));

      if (
        this.isHandleParamType === "userDefined" ||
        ...
        data.map((item) => {
          const prs = {
            ...
            apEnum: flag ? item.apEnum : item.dataEnum,
    ...
    paramKeyBlur(data) {
      ...
      if (data.paramValueType == "List") {
        ...
        else if (!regList.test(data.paramKey))
          return this.$message.error("参数key只能以英文开头，List结尾");
      } else if (data.paramValueType == "Object") {
        if (this.javaKeyWord.includes(data.paramKey))
          return this.$message.error("参数key不能是Java关键字");
        else if (data.paramKey == "")
          return this.$message.error("参数key不能为空");
        else if (!reg.test(data.paramKey))
          return this.$message.error(
            "参数key以英文开头,或数字结尾,错误输入,例如：A8A、A8A8"
          );
      } else {
        if (data.paramKey == "") return this.$message.error("参数key不能为空");
        else if (!reg.test(data.paramKey))
    ...
    setLengthDisabled(data) {
      if (
        ["Boolean", "Character", "boolean", "char"].includes(
          data.paramValueType
        ) &&
        data.paramCustomFlg == "1"
      ) {
        data.lengthDisabled = true;
        data.paramLength = "1";
      } else if (
        ["Object", "List"].includes(data.paramValueType) &&
        data.paramCustomFlg == "1"
      ) {
        data.lengthDisabled = true;
        data.paramLength = "";
    ...
    showDialogBody(e) {
      this.bodyFlag = e;
      this.paramCustomFlg = false;
      this.$refs.bodyDefineRef.showDialog(this.dataObj.beltLine);
    },
    //自定义body保存
    saveBodyParams(e) {
      console.log(e, "0----000");
