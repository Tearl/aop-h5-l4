<template>
  <div class="aop_tradedesign_comps_dicmgmt_dic_list">
    <TableHeader
      title="数据字典管理"
      :btnList="btnList"
...
export default {
  ...
  methods: {
    ...
    async operate(operation, data) {
      if (operation == "delete") {
        this.$confirm("确定要删除吗？").then(async () => {
          ...
          const res = await this.rpc.systemmgmt.delSysDic(params);
          this.$notify({
            title: "成功",
            message: "删除成功",
            duration: 2000,
    ...
    async getList() {
      ...
      const api = this.pageType == "sys" ? "getSysDicList" : "getSysDictionaryList"
    ...
    handleBtnClik(btn) {
      switch (btn) {
        ...
        case "import":
          ...
          this.actionUrl = this.rpc.file.sysDicUpUrl({ isCover: "1", sysId: this.sysId });
          return;
        case "export":
          if (
            this.selectedData.length == 0 &&
    ...
    radioChange(e) {
      if (e == "1") {
        ...
        this.actionUrl = this.rpc.file.sysDicUpUrl({ isCover: "0", sysId: this.sysId });
      }
    },
    // 多选框变化时
    ...
    async exportDic() {
      ...
      const res = await this.rpc.systemmgmt.exportSysDic(params);
      const downLoadUrl = this.$replaceUrl(res.downLoadUrl);
      const a = document.createElement("a");
      a.style.display = "none";
      a.download = "";
    ...
    // 初始化
    init() {
      this.sysId = this.sysId ? this.sysId : this.$route.query.sysId
      this.getList();
...
.aop_tradedesign_comps_dicmgmt_dic_list {
