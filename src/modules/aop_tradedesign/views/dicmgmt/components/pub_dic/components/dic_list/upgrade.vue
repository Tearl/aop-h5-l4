<template>
  <div class="aop_tradedesign_comps_dicmgmt_sys_dic_list_upgrade">
    <PageDialog
      ref="upgradeDialog"
      dialogWidth="30%"
      dialogTitle="数据字典升级"
...
export default {
  ...
  methods: {
    ...
    async upgrade() {
      ...
      const res = await this.rpc.systemmgmt.upgradeClassify(params);
      this.$notify({
        title: "成功",
        message: "数据字典升级成功",
        duration: 2000,
    ...
    async check() {
      this.$refs.editForm.validate(async (valid) => {
        ...
        const res = await this.rpc.systemmgmt.upgradeSysDic(params);
...
.aop_tradedesign_comps_dicmgmt_sys_dic_list_upgrade {
  .upgrade_dialog {
    .dialog_content {
      text-align: center;
      .transfer_form {
