<template>
  <div class="aop_tradedesign_comps_dicmgmt_pub_dic_list">
    ...
    <div v-if="flag == 'view'" class="content_wrapper_box">
      <DicView :type="attributeTyp" :dictryId="dictryId" @toDicList="toDicList" dicType="pubView"></DicView>
    </div>
    ...
    <PageUploadBoxMetaData
      dialogTitle="导入元数据"
      :dialogVisiable="importMetaDataVisible"
      dialogWidth="30%"
      :actionUrl="actionUrl"
      :data="{beltLine: beltLine}"
      @uploadFile="uploadFile"
      @closeDialog="importMetaDataVisible = false"
      @download="download"
      @refresh="getList"
      @radioChange="radioChange"
    ></PageUploadBoxMetaData>
    ...
    <PagePop
      desc="确定全量导出数据吗"
      :isPopCompleteShow="isExportAllPop"
      @handleCancel="closeExportAllPop"
      @handleConfirm="exportAll()"
    ></PagePop>
...
import PageUploadBoxMetaData from "@m/core/components/page_upload_box/importMetaData"
...
export default {
  ...
  data () {
    return {
      ...
      attributeTyp: '',
      //全量导出数据
      isExportAllPop: false,
      importMetaDataVisible: false,
    }
  },
  methods: {
    // 点击搜索按钮
    ...
    async operate (operation, data) {
      ...
      if (operation == "delete") {
        ...
        this.$nextTick(() =>{
          this.flag = "view"
          this.attributeTyp = data.attributeTyp
        })
        // this.$emit("toDetail", false)
        // this.$emit("getDataDictionProd", { v: "公共数据字典详情", k: "dd", closable: true },data.dictryId)
    ...
    handleBtnClik (btn) {
      switch (btn) {
        ...
        case "export":
          ...
          return
        case "importAll":
          this.importMetaDataVisible = true
          this.actionUrl = this.rpc.file.importMetaDataUrl()
          return
        case "exportAll":
          this.isExportAllPop = true;
          return
        case "rule":
    ...
    async exportAll() {
      const res = await this.rpc.dicmgmt.exportMetaData({
        beltLine: this.beltLine
      })
      const businessDomainInfo = this.businessDomainLists.find(item => item.bizDomainNo == this.beltLine)
      const filename = businessDomainInfo?.bizDomainName ? `${businessDomainInfo?.bizDomainName}元数据.json` : "元数据.json"
      const data = res.metaDataInfo
...
.aop_tradedesign_comps_dicmgmt_pub_dic_list {
  // display: flex;
  // padding: 16px;
  // padding-left: 332px;
  .content_wrapper {
