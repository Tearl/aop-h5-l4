<template>
  <div class="aop_tradedesign_dic_cmps_my_sub_list">
    <div class="D2_workspace_table_list">
      <div class="table_operate">
        <div class="header_content">
          <h1>分段列表{{ buttonList }}</h1>
      ...
      >
        ...
        >
          <el-table-column
            ...
            v-if="selectedKey.includes('dataSubNo')"
          ></el-table-column>
          <el-table-column
            label="分段名称"
            prop="dataSubNm"
          ...
          <el-table-column
            ...
            show-overflow-tooltip
          ></el-table-column>
          <el-table-column label="状态" prop="dataSubStatus">
            <template slot-scope="scope">
              <span
                :class="['status_span', `status_${scope.row.dataSubStatus}`]"
                >{{ publishStatus[scope.row.dataSubStatus] }}</span
              >
            </template>
          </el-table-column>
          <!-- <el-table-column
              ...
              <div class="application_manage_list_card_item_top">
                ...
                <div class="application_manage_list_card_item_top_desc">
                  ...
                  <div class="application_manage_list_card_item_platform">
                    <span class="app_auth">{{
                      item.beltLineNm || objTemp[item.beltLine]
                    }}</span>
                    <span class="app_publish">{{
                      item.classifyId | classifyIdSt
                    }}</span>
                  </div>
                </div>
              </div>
...
export default {
  mixins: [mixin],
  components: { commonTableOperate, ReviewDialog,Table },
  props: {
    active: {
      type: String,
      default: "01",
  ...
  data() {
    return {
      ...
      selectedKey: ["dataSubNo"],
      selectItemKeyList: ["dataSubNo"],
      toggleIndex: 0,
      // 待选项的数据列表
      tableColumnList: [
        {
  ...
  methods: {
    ...
    async getDicList(e) {
      ...
      const res = await this.rpc.dicmgmt.getSubFieldPage(queryParams);
      console.log("res------======",res)
      this.list = res.list || [];
...
.aop_tradedesign_dic_cmps_my_sub_list {
  .el-table__header,
  .el-table__body {
    overflow: hidden;
