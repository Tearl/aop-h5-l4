<template>
  <div class="aop_tradedesign_comps_dicmgmt_dialog_selcParam">
    <!-- 从数据集中选择 -->
    <PageDialog
      dialogTitle="请选择参数"
          ...
          <div class="check_content">
                  ...
                  >
                    <span class="custom-tree-node" slot-scope="{ node, data }">
                      <span
                        ...
                        >{{ `${node.label}` }}</span
                      >
                      <div v-if="node.level == '1'" class="disabled" @click.stop></div>
                    </span>     
                    </el-tree
                  >
                </div>
            ...
            <div class="middle">
              <div class="right_table">
                ...
                <div class="content">
                  <div class="table">
                    <div class="title">{{ `字段列表（${total}）` }}</div>
                    <Table
                      class="table_style"
                      :data="dictList"
                      :table="tableColumn"
...
export default {
  ...
  methods: {
    ...
    async getFiledList() {
      ...
      this.total = res.turnPageTotalNum;
      this.$nextTick(() => {
        this.isTableData();
      });
    },
    ...
    saveParams() {
      ...
      this.closeDialog();
      this.$emit("saveParams", this.tableData, this.currentSubObj,"field");
    },
    //分段树点击
...
.aop_tradedesign_comps_dicmgmt_dialog_selcParam {
  .add_dialog_conf {
    padding: 15px 12px;
    .search_conf {
      padding: 0;
