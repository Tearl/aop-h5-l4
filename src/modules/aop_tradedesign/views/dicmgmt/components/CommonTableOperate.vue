<template>
  <div class="common_table_operate">
    <div
      @mouseleave="item.hasActive = false"
      @mouseenter="item.hasActive = true"
      @click="btnClick(i)"
      :class="['common_table_operate_item']"
      v-for="(item, i) in operate_btn"
      :key="item"
    >
      <el-popover
        placement="bottom"
...
export default {
  ...
  methods: {
    ...
    handleChange(value) {
      console.log(value);
      // if(this.selectItemKeyList.length === 0) {
      //     this.$message({
      //     message: '至少需要选择一项',
      //     type: 'error',
      //     duration: 1000
