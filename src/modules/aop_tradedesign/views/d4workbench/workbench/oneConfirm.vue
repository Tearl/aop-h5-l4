  ...
  <div class="aop_tradedesign_page_d4workbench_workbench_one_confirm">
    ...
    <div class="application_detail_tabs">
      <div class="application_detail_tabs_label_area">
        ...
        <span class="fnc_Nm">{{ dataObj.funcNm }}</span>
      </div>
    </div>
    ...
    <div class="content">
      ...
      <div class="content_header">
        ...
        >
          <template v-slot:child>
            <div class="tabs_btn_style" v-if="dataObj.type != 'look'">
              <el-button type="primary" @click="designDetail" class="detail"
                >页面设计详情</el-button
              >
              <el-button type="primary" @click="commitAsset"
                >关闭当前窗口</el-button
              >
            </div>
            <div class="tabs_btn_style" v-else>
              <el-button type="primary" @click="commitAsset"
                >资产确认信息查看</el-button
              >
            </div>
      ...
      <div class="record_btn_style" v-show="defaultActive == 'processTabs'">
        <el-button type="text" @click="handleBackBtn" v-show="fourRAttrFlag"
          >返回</el-button
        >
        <el-button type="text" @click="problemRecord" v-show="showProm">
      ...
      <div v-show="defaultActive == 'processTabs'">
        <div
          ...
          :class="dataObj.type == 'look' ? 'look_contain_box' : 'contain_box'"
        >
    ...
    <PageFooter v-show="this.dataObj.type != 'look'">
      <div slot="box">
        <el-button @click="confirm" type="primary">交易确认</el-button>
      </div>
    </PageFooter>
...
import Detail from "./components/download_design/components/one_confirm";
import PageFooter from "@m/core/components/page_footer";
import commitDialog from "./components/download_design/components/one_confirm/components/commit";
import ProblemRecord from "./components/download_design/components/one_confirm/components/problem_record";
...
export default {
  ...
  methods: {
    init() {
      ...
      this.dataObj.type = this.dataObj.type ? this.dataObj.type : "edit";
    ...
    async getD4bServiceInfo() {
      ...
      this.dataObj.businessId = res.svcId;
      this.dataObj.businessName = res.svcNm;
      this.recordTitle = res.svcNm;
    },
    //流程图
    ...
    getAttrEvent(e, click) {
      ...
      this.getAttrEventTimer = setTimeout(() => {
        if (
          ["D4bAction", "D5SelfAction", "D5LinkAction"].includes(e.actionType)
        ) {
          this.chooseType = "action";
          this.recordTitle = e.actionName;
          this.expend = true;
          this.dataObj.svcId = e.actionOwnId;
          this.dataObj.svcName = e.actionName;
          this.showProm = true;
    ...
    //点击画布空白
    getSvgClickEvent(e) {
      console.log("点击空白", e, this.showProm);
      if (
    ...
    confirm() {
      this.$refs.commitDialogRef.showDialog(this.dataObj);
    },
    async confirmReview(data) {
      let arr = [];
      ...
      data.map((item) => {
        arr.push(
          Object.assign({}, item, {
            ...
            funcName: this.dataObj.funcNm,
            approvalType: "01",
            problemType: "03",
            layoutId: this.dataObj.layoutId,
            layoutName: this.dataObj.layoutName,
            cpntId: this.dataObj.cpntId,
            cpntName: this.dataObj.cpntName,
            svcId: item.svcId || "",
            svcName: item.svcId ? item.svcName : "",
            compositeId: this.dataObj.compositeId,
            compositeName: this.dataObj.compositeName,
            eventId: this.dataObj.eventId,
            eventName: this.dataObj.eventName,
            detailType: item.detailType,
            opinion: item.opinion,
            passStatus: item.passStatus,
            rarId: this.dataObj.rarId,
            businessId: this.dataObj.businessId || "",
            businessName: this.dataObj.businessName || "",
          })
        );
      });
      const params = {
        questionRecordInfoList: arr,
      };
      const res = await this.rpc.d4.addProblemRecord(params);
      this.$notify({
        title: "成功",
        message: "提交成功",
        duration: 2000,
      ...
      this.$refs.commitDialogRef.closeDialog();
    },
    problemRecord() {
      this.$refs.probRecordRef.showDialog(this.recordTitle, this.dataObj.type);
    },
    async confirmProblem(data) {
      let arr = [];
      console.log("data", data);
      data.map((item) => {
        arr.push(
          Object.assign({}, item, {
            ...
            cpntId: this.dataObj.cpntId,
            cpntName: this.dataObj.cpntName,
            svcId: item.type == "35" ? this.dataObj.svcId : "",
            svcName: item.type == "35" ? this.dataObj.svcName : "",
            compositeId: this.dataObj.compositeId,
            compositeName: this.dataObj.compositeName,
            eventId: this.dataObj.eventId,
            eventName: this.dataObj.eventName,
      ...
      this.$refs.probRecordRef.closeDialog();
    },
    commitAsset() {
      window.close()
      // let action = this.dataObj.type == "look" ? "look" : "confirm";
      // location.href = `/h5-pc-terminal-web/layout_preview/#/reviewex?reviewId=${this.dataObj.rarId}&approvalType=01&orderId=${this.dataObj.isolateWorkspace}&funcId=${this.dataObj.funcId}&modelVersion=${this.dataObj.modelVersion}&action=${action}`;
    },
    designDetail() {
      window.open(
        `/h5-pc-terminal-web/layout_preview/#/previewx?kind=01&orderId=${this.dataObj.isolateWorkspace}&funcId=${this.dataObj.funcId}&modelVersion=${this.dataObj.modelVersion}&layoutId=${this.dataObj.layoutId}`
      );
    },
    handleClose() {
      this.expend = false;
