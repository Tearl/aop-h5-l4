...
export default {
  ...
  watch: {
    sendData(val) {
      val.forEach((item) => {
        if (!item.paramParentId && item.paramPosition == "0") {
          item.childTier = 1;
  ...
  methods: {
    ...
    initParams(data) {
      ...
      this.sendData.forEach((item) => {
        ...
        if (item.paramPosition == "0" && item.children) {
          item.showChild = true;
    ...
    paramKeyBlur(e) {
      ...
      if(e.paramValueType == "List"){
        if (e.paramKey == "") return this.$message.error("参数key不能为空")
        else if(!regList.test(e.paramKey)) return this.$message.error("参数key只能以英文开头，List结尾")
      }else if(e.paramValueType == "Object"){
        if(this.javaKeyWord.includes(e.paramKey)) return this.$message.error("参数key不能是Java关键字")
        else if (e.paramKey == "") return this.$message.error("参数key不能为空")
        else if (!reg.test(e.paramKey)) return this.$message.error("参数key以英文开头,或数字结尾,例如AA、A88,不能是A8A8")
      }else{
        if (e.paramKey == "") return this.$message.error("参数key不能为空")
        else if(!reg.test(e.paramKey)) return this.$message.error("参数key以英文开头,或数字结尾,例如AA、A88,不能是A8A8")
      }
    ...
    //确认选择参数
    async saveParamsDataset(tableData,addType=false) {
      this.tableLength = 0;
