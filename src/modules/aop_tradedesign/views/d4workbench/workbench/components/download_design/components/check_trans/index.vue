<template>
  <div class="aop_tradedesign_comps_d4editor_service_define_check_dialog">
    ...
    <PageDialog :dialogWidth="'720px'" :dialogTitle="'详情'" :dialogVisiable="standardCheckDetailVisible"
      ...
      <div slot="box">
        ...
        <div v-if="currentShowTitle == 'L5服务未被完全编排'">
          <div class="title">未被编排L5服务列表</div>
          <Table class="table" :data="serviceCheckBeanList" :table="serviceCheckBeanColumn"></Table>
        </div>
      </div>
      <div slot="footer">
...
export default {
  ...
  data() {
    return {
      ...
      successFlag: "",
      checkEnd: false,
      serviceCheckBeanList:[],  //L5服务未被完全编排
      serviceCheckBeanColumn:[
        {
          prop: "serviceId",
          label: "服务ID",
          minWidth: "50%",
  ...
  methods: {
    ...
    async showStandardCheckDialog(data) {
      ...
      setTimeout(() => {
        // L5服务未被完全编排
        this.serviceCheckBeanList = data.serviceCheckBeanList
        this.standardCheckData[4].checkFlg = data.l5PreliminaryCheck
        this.checkEnd = true
        let flag = this.standardCheckData.filter(item => item.checkFlg != "1")
        this.successFlag = flag.length ? false : true
      }, 2000)
...
.aop_tradedesign_comps_d4editor_service_define_check_dialog {
