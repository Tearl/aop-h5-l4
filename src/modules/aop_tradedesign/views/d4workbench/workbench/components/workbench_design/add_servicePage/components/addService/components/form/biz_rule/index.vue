<template>
  <div class="aop_tradedesign_comps_d4_edit_biz_rule">
    <div class="header_title">业务规则</div>
    <div class="content_wrapper" v-if="isEdit">
      <div class="block">
        <div class="title">业务规则表：</div>
        <RuleTable
          ref="ruleTableRef"
          :apiId="apiId"
    ...
    <div class="content_wrapper" v-else>
      ...
      <div class="block">
        <div class="title">规则说明</div>
        <div class="content">{{ ruleInfo.ruleComnt }}</div>
      </div>
    </div>
    <PageDialog
...
export default {
  ...
  methods: {
    ...
    confirm() {
      this.$refs.bizRuleFormRef.validate((valid) => {
        ...
        if (this.dialogTitle == "添加规则说明") {
          ...
          const target = this.businessRuleList.find(
            (item) => item.ruleNo == this.isData.ruleNo
          );
          const i = this.businessRuleList.indexOf(target);
          this.$set(
            this.businessRuleList,
            i,
            JSON.parse(JSON.stringify(this.bizRuleForm))
          );
        }
        this.bizRuleVisible = false;
        this.$refs.bizRuleFormRef.resetFields();
    ...
    getRuleAdd(){
      this.$refs.ruleTableRef.addBusinessRule();
    },
    init() {
      this.$refs.ruleTableRef.setRuleInfo({})
    }
  },
  created() {},
  mounted() {
