...
export default {
  ...
  methods: {
    ...
    //编辑保存
    saveService(){
      console.log("服务信息编辑保存")
    },
    async getBusinessRuleInfo() {
      this.ruleInfo = {}
      const params = {
        apiId: this.basicData.svcId,
      };
      ...
      this.$nextTick(() =>{
        this.$refs.bizRuleRef.ruleInfo = res;
      })
    },
    // 初始化
    init() {
  ...
  watch: {
    async basicData(res, o) {
      ...
      this.iOutputList = [];
      this.$nextTick(() => {
        console.log(sessionStorage.getItem("dataInfo"),'sessionStorage.getItem("dataInfo")')
