<template>
  <div class="aop_tradedesign_download_design_data_set_comps_add_user_undefined">
    <PageDialog
      :dialogWidth="'70%'"
      :dialogTitle="'请选择元数据'"
...
import FiledForm from "../form/filedForm";
export default {
  mixins: [mixin],
  components: {
    Table,
  ...
  methods: {
    ...
    saveParams() {
      ...
      if (this.paramsDefault == "add") {
        this.$refs.filedRef.confirm();
      } else {
        if(Object.keys(this.dataObj).length==0) return this.$message.error("请选择数据")
        // let arr = this.filterType(this.tableData);
        this.dataObj.dataSubstnNo = this.dataSubObj.dataSubstnNo;
        this.dataObj.dataSetNo = this.dataSubObj.blgDataSetNo;
        this.chooseFiled();
      }
    },
    async chooseFiled() {
      const params = Object.assign({}, this.dataObj);
      const res = await this.rpc.dicmgmt.addMyField(params);
      this.$notify({
        title: "成功",
        message: "成功",
        duration: 2000,
      ...
      this.$emit("saveParams", this.dataObj);
      this.handleCancel();
    },
    addParams() {
      this.$emit("confirmField", this.dataObj);
      this.handleCancel();
    },
...
.aop_tradedesign_download_design_data_set_comps_add_user_undefined {
}
