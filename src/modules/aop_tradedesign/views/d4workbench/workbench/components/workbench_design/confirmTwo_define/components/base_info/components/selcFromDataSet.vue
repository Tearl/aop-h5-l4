...
export default {
  ...
  methods: {
    showDialog() {
      ...
      this.tableData = [];
      this.currentSer = JSON.parse(sessionStorage.getItem("servInfo"));
      console.log(this.currentSer,"传值");
      this.getDetail();
    },
    reFresh(e, type) {
       if (type == "field") {
    ...
    async getDetail(){
      const params = {
        dataSetNo:this.currentSer.dataSetNo,
      };
    ...
    async getSubsList() {
      const params = {
        ...
        dataSetNo: this.dataObj.dataSetNo || this.currentSer.dataSetNo,
        currentPage: "1",
        turnPageShowNum: "99",
      };
