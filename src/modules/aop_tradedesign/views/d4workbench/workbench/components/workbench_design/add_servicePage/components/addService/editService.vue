...
export default {
  ...
  methods: {
    getList() {
      this.$refs.datasetDetailRef.getDatasetList();
    },
    init() {
    ...
    showDialog(data) {
      ...
      this.getApiDetail(data);
      this.getParamsList();
      // this.getBusinessRuleInfo(data)
    ...
    async getApiDetail(data){
      ...
      if(res.apiServiceType !== "D5自有服务"){
        ...
        console.log("res------>",this.currentServObj)
        this.getDetail(res);
        this.getBusinessRuleInfo()
      }
      obj ={
        ...
        svcNm:res.apiName,
        svcNo:res.apiCode,
        dataSetNm:this.radioObj.dataSetNm,
        svcDesc:res.apiDesc,
      ...
      this.$refs.basicRef.$refs.formRef.validate(async (valid) => {
        ...
        if (form.svcTyp == "D5自有服务") {
          ...
          const params = {
            ...
            // ruleComnt,
            // columnInfoList,
            dataSetNo:this.radioObj.dataSetNo,
            dataSetNm:this.radioObj.dataSetNm,
            apiName:form.svcNm,
            apiDesc:form.svcDesc,
            apiServiceType:form.svcTyp || this.radioObj.apiServiceType,
            puApiParams: puApiParams,
          ...
          if(this.ruleInfo.ruleDefnId){
            const that = this.$refs.bizRuleRef.$refs.ruleTableRef;
            // console.log(that.tableColumn,that.tableData,this.subTableColumn,"123")
            // let flag = that.tableColumn.some((item) => item.isSubColumn == true)
            // if(!flag || !that.tableData.length > 0) return this.$message.error("业务规则不能为空")
            // let flag = that.getCheckData;
            // console.log(flag,"12344")
    ...
    async getParamsList() {
      ...
      let mapData = new Map();
      this.paramsIdList.map((item) =>{
        mapData.set(item.dataSetDataId,1)
      })
      this.mapDataList = mapData
      // console.log(mapData,this.mapDataList,this.paramsIdList,'mapData')
    },
    addParamsId(data) {
    ...
    hasKeyword(data) {
      data.map(item => {
        if(item.paramCustomFlg == "1" && item.paramValueType == "Object" && this.javaKeyWord.includes(item.paramKey) ) {
