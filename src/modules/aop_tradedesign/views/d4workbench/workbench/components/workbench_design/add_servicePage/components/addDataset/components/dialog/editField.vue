...
export default {
  ...
  computed: {
    showEnum() {
      enumCondition(this.formData.dictryTyp)
    }
  },
  methods: {
    async showFiledDialog(e) {
      ...
      // })
      if(e){
        Object.assign(this.fieldInfo,e)
        await this.getSubOption();
    ...
    async getDetail() {
      ...
      if(enumCondition(res.dictryTyp)){
        if(res.dictryEnum && res.dictryEnum !== ""){
          this.$refs.enumRef.enumerateValueList = res.dictryEnum.split(",").map((item) =>{
            let arr = item.split("-");
            return {
              key:arr[0],
              value:arr[1],
    ...
    async validator(rule, value, callback) {
      if (rule.field == "dictryLength") {
        ...
        if(value == "" || value && !reg.test(value)){
          callback(new Error("请输入以英文开头,或数字结尾"))
        }else{
