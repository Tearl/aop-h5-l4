<template>
  <div class="aop_tradedesign_comps_design_elem_d5_self_pick_list">
    ...
    <ul class="api_info_list">
      <li v-for="(item, index) in apiInfoList" :key="item" class="api_info_list_item">
        <div class="left" @mousedown.stop="pickOne(item, $event)">
          ...
          <el-tooltip
            ...
            placement="top"
          >
            <div class="service_name bold_text">{{ item.apiName }}</div>
          </el-tooltip>
    ...
    <EditService ref="editServiceRef" @confirmService="confirmService" :datasetDisabled="true"></EditService>
  </div>
</template>
...
import EditService from '../../../../../add_servicePage/components/addService/editService'
export default {
  mixins: [mixin],
  components:{
    EditService
  ...
  methods: {
    ...
    async loadNode(node, resolve) {
      ...
      if (node.level === 1) {
        ...
        const res = await this.rpc.public.getGateServiceList(params);
    ...
    async getServGrpgList(arsId) {
      ...
      const res = await this.rpc.public.getReleaseApiGroupList(params);
    ...
    async getAssociatedSelfServiceList() {
      ...
      this.aMenuList = res.apiInfoList.map((item) => {
    ...
    async del(data) {
      this.$confirm("是否删除该服务，服务名称为" + data.apiName)
        .then(async () => {
          ...
          const res = await this.rpc.dicmgmtDefine.checkServiceDel(params);
          this.$notify({
            title: "成功",
            message: "删除成功！",
            duration: 2000,
...
.aop_tradedesign_comps_design_elem_d5_self_pick_list {
  display: flex;
