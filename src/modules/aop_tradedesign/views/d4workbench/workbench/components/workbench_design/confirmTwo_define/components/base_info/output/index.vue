  ...
  <div class="aop_workgate_comps_apimgmt_api_mod_params">
        ...
        <section class="d_flex_center">
          <el-radio-group v-model="radioHeader">
            <el-radio-button label="0"
              ><p class="radio-text">选择现有数据字典参数</p></el-radio-button
            >
            <el-radio-button label="1"
              ><p class="radio-text">新增数据字典</p></el-radio-button
            >
          </el-radio-group>
        </section>
    ...
    >
      ...
      <span slot="footer">
        ...
        <el-button type="primary" @click="confirm('enumerateEdit')"
          >确 定</el-button
        >
      </span>
      >
...
import SelectFromDataSet from "../components/selcFromDataSet";
...
export default {
  ...
  methods: {
    ...
    async getRuleClassification() {
      ...
      this.addRuleServiceForm.find(
        (item) => item.model == "paraClassifyId"
      ).select = selectList;
      this.addRuleServiceForm.find(
        (item) => item.model == "paraClassifyId"
      ).select = selectList;
      ...
      if (type == "enumerateEdit") {
        this.$refs.enumerateValueFormRef.validate((valid) => {
          ...
          if (this.dialogTitle == "添加规则说明") {
            ...
            _this.enumerateValueList[i].key =
              this.enumerateValueForm.enumerateValue;
            _this.enumerateValueList[i].value =
              this.enumerateValueForm.stepContent;
          }
          this.enumerateValueVisible = false;
          this.$refs.enumerateValueFormRef.resetFields();
        });
        ...
        const _this = this.$refs.dictionaryFormRef.$refs.ruleRef;
        _this.ruleTableData = unduplicated(
          _this.ruleTableData.concat(this.tableData),
          "ruleId"
        );
        this.addRuleDialogVisible = false;
      }
    ...
    // 保存当前选中的参数
    async saveParamsData() {
      const selectData = this.$refs.dataSetRef.tableData;
