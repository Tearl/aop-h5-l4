<template>
  <div class="aop_tradedesign_download_design_data_set_comps_filed_form">
    <el-form
      :model="formData"
      :rules="rules"
      class="edit_form"
      ...
      <div class="service_create_form_info">
        <el-form-item label="所属数据集：">
          <span class="text_style">{{
            dataInfo.dataSetNm || formData.dataSetNm
          }}</span>
        </el-form-item>
        <el-form-item label="所属分段：" prop="dataSubstnNo">
          ...
          <!-- <span class="text_style">{{ subObj.dataSubstnNm }}</span> -->
        </el-form-item>
        ...
        <el-form-item
          label="最大长度："
          prop="dictryLength"
          v-if="formData.dictryTyp != 'List' && formData.dictryTyp != 'Object'"
        >
          <el-input
        ...
        <el-form-item label="L5服务" class="no_before" v-show="type == 'edit'">
          <ServList
            bizType="read"
            :servList="servList"
            :infoData="infoData"
...
import Enumerate from "../dialog/enumerate";
import { 
  enumCondition
...
export default {
  ...
  methods: {
    showFiledDialog(e) {
      ...
      this.formData.dataSubstnNo = e.dataSubstnNo;
      this.$nextTick(() => {
    ...
    initFrom() {
      this.clearData()
      this.getSubOption();
      this.formData.dataSubstnNo = this.dataInfo.dataSubstnNo;
    },
    async getSubOption() {
      const params = {
        dataSetNo: this.dataInfo.blgDataSetNo,
    ...
    async addDictionary() {
      ...
      params.dataSetNo = this.dataInfo?.blgDataSetNo;
      // params.dataSubstnNo = this.subObj?.dataSubstnNo;
      // 从字段池选择时的传参
      const addParams = {
...
.aop_tradedesign_download_design_data_set_comps_filed_form {
