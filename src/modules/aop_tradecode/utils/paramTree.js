...
export function packageParamKey(param, paramList, paramKey = "") {
  ...
  if (paramOrder != paramParentId && paramParentId) {
    ...
    console.log(param.paramKey, 'param.paramKey')
 ...
 * @return {*}
 */
export function setParamOrder(arr, paramOrder, paramParentId = "", key = "paramOrder", childKey = "children", paramParentKey = 'paramParentId') {
  if (arr.length == 0 || !arr) return []
  return arr.map((item, i) => {
    if (item[childKey] && item[childKey].length > 0) {
      item[key] = paramOrder ? `${paramOrder}.${i + 1}` : `${i + 1}`;
      item[paramParentKey] = paramParentId;
      setParamOrder(item[childKey], item[key], item[key], key, childKey, paramParentKey);
      return item
    } else {
      item[key] = paramOrder ? `${paramOrder}.${i + 1}` : `${i + 1}`;
      item[paramParentKey] = paramParentId;
...
export function rowInfos(row){
    if(!row){return};
    const params = JSON.stringify(row);
    const params2 = params.replace("{", "");
    const params3 = params2.replace("}", "");
    const params4 = params3.replace(/:/g, "=");
    const params5 = params4.replace(/,/g, "&");
    return params5.replace(/"/g, "");
}
