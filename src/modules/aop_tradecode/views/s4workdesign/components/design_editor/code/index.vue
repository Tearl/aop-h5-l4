    ...
    <div class="design_container">
      ...
      <div class="design_container_process">
        <div class="design_container_process_left" ref="leftMove">
          ...
          <!-- commit -->
          <div v-if="operateType === 'commit' || operateType === 'conflict'">
            <el-input
              class="commit-info"
              type="textarea"
              :autosize="{
        ...
        <div class="design_container_process_middle" ref="moveDiv"></div>
        <div class="design_container_process_right" ref="rightMove">
          <!-- pull merge -->
          <div class="design_container_process_right_button">
            ...
            >
              <img
                :src="require('@m/assets/images/git/logout.png')"
                @click="handleLogout()"
                alt=""
              />
            </el-tooltip>
            ...
            >
              <img
                :src="require('@m/assets/images/git/git-commit.png')"
            ...
            >
              <img
                :src="require('@m/assets/images/git/git-merge.png')"
            ...
            <el-tooltip
              ...
              content="刷新源码"
              placement="top"
              v-if="operateType !== 'merge' && operateType !== 'conflict' && !isDisabled"
            >
              <img
                :src="require('@m/assets/images/git/refresh.png')"
                @click="updateCode()"
                alt=""
              />
            </el-tooltip>
            ...
            <el-tooltip
              ...
              content="运行日志"
              placement="top"
              v-if="operateType !== 'merge' && operateType !== 'conflict'"
            >
              <img
...
import demo from "./components/diguidemo.vue";
...
export default {
  ...
  methods: {
    ...
    dealCode(diffCode, oldCode) {
      ...
      while (list.length > 1) {
        ...
        if (startLine === -1 && endLine === -2) {
          ...
          this.replaceLines2(newCode, startLine, endLine, oldCodeList);
        }
    ...
    async openFile(item) {
      ...
      if (this.operateType === "conflict") {
        for (let i = 0; i < this.conflictList.length; i++) {
          const tab = this.conflictList[i];
          // 如果打开过，则使用之前的代码
          console.log("sssssss", item.id, tab.id);
    ...
    handleChange(e) {
      ...
      for (let i = 0; i < this.editableTabs.length; i++) {
        ...
        if (tab.path === this.editableTabsValue) {
          ...
          } else if (!this.operateType || this.operateType === "merge") {
      ...
      const list = res?.files?.map(async (item) => {
        ...
        item?.sections.map((section, sectionsIndex) => {
          ...
          lines.map((data, index) => {
            ...
            if (!section.conflict) {
              if (data.text.includes("@@") && data.type === "match") {
                // @@ A @@ B
            ...
            if (section.conflict && index === section.lines.length - 1) {
              code += ">>>>>>>" + "";
            }
          });
    ...
    getSaveCodeList() {
      ...
      this.rpc.assetList.queryApplicationSaveCodeList(params).then((res) => {
    ...
    async getFunctionBranch() {
      ...
      this.newId = this.newId?this.newId:res.applyInfoBranchList[0].appId;
