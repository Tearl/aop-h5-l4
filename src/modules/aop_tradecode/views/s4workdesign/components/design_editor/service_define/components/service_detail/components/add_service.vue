...
export default {
    ...
    methods: {
        handleClose(type,val) {
            if(this.showServiceTyp && this.showServiceTyp =='changeSvr'){
                this.$emit("closeDialog",'close',this.isCheckList);
                return;
            }  else {
                this.$emit("closeDialog",'onlyClose',val);
            }
        },
        ...
        async saveTradeAchieveSituation(data,type){
            // 添加接口实现
            if(this.showServiceTyp && this.showServiceTyp =='changeSvr'){
                this.$emit("closeDialog",'addsvrClose',this.isCheckList);
                return;
            }
            // 一个交易 多个接口
            let list = this.isCheckList.map((item)=>{
                return {
                    ...
                    txId: this.detailInfo.svcId,
                    txNm: this.detailInfo.svcNm,
        ...
        async getApplication(row){
            const params = {
                fnctCd: this.formB.assetFnctNo, //功能编号
                authAstFnctInfoId: this.$route.query.authAstFnctInfoId || this.formB.authAstFnctInfoId
            }
            this.rpc.assetList.intQueryProjectIdNameInfo(params).then((res)=>{
                // console.log("查询工程应用名称",res)
                let item = {
                    authFnctJobInfoId: res.authFnctJobInfoId,
                    blgPojAppId: res.blgPojAppId,
                    blgPojAppNm: res.blgPojAppNm,
                }
