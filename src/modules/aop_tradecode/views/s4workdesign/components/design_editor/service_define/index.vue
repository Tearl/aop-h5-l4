            ...
            >
                ...
                >
                    ...
                    <el-button
                        ...
                        type="primary"
                        size="small"
                        @click="connectListOperate('configured')"
                        >工程配置</el-button
                    >
                    <el-button
                        plain
                    ...
                    <div class="process_container">
                        ...
                        >
                            ...
                            <div class="process_middle_wrapper">
                                ...
                                />
                                <div
                                    class="top-tools-btns"
                                    v-if="asideActive === '2' && $route.query.displayType != '01'"
                                >
                                    <div class="left-btns">
...
import BusinessRules from "../BusinessRules/index.vue";

import exportFnctHistory from "@m/core/components/exportFnctHistory/index.vue";
export default {
    mixins: [mixin],
    components: {
        Pattr,
    ...
    methods: {
        ...
        async changeEdit() {
            this.isEdit = true;
            await this.$refs.baseInfoRef.$refs.basicRef.changeEdit("edit");
        },
        async saveBasic() {
            const res = await this.$refs.baseInfoRef.$refs.basicRef.changeEdit("save");
            if (!res) 
            this.isEdit = false;
        },
        //初始化
        init() {
            if (this.$route.query.typeOf4b == "1") {
                // 从后端接口列表过来
            ...
            this.assetFnctNo = this.$route.query.fnctId; // 功能号
        ...
        pickElemEvent(e) {
            if (e.item.referId != "executeFlowAction") {
                ...
                (this.elemActive =
                    "业务交易" ||
                    this.defaultActive == "businessDecisionProcess")
            ) {
                this.$refs.pattr.showType = "service";
            } else if (
                (this.elemActive =
                    "接口列表" || this.defaultActive == "businessProcess")
            ) {
                this.$refs.pattr.showType = "service";
            }
        },
            ...
            if (e.ctrlKey) {
                if (e.wheelDelta > 0) {
                    if (this.scaleRateFlow < 2) {
                        this.scaleRateFlow =
                            (this.scaleRateFlow + 0.1).toFixed(1) * 1;
                    }
                } else {
            ...
            if (e.ctrlKey) {
                if (e.wheelDelta > 0) {
                    if (this.scaleRateFlow4b < 2) {
                        this.scaleRateFlow4b =
                            (this.scaleRateFlow4b + 0.1).toFixed(1) * 1;
                    }
                } else {
        ...
        async getD4bServiceList(data) {
            ...
            if (this.$route.query.svcId) {
                ...
                this.currentD4bObj =
                    this.d4bSvcList.find(
                        (item) => item.svcId == this.$route.query.svcId
                    ) || {};
                ...
                this.currentD4bObj =
                    this.d4bSvcList.find(
                        (item) => item.svcSeq == this.serviceSeq
                    ) || {};
            } else {
                let svcId = getSessionStorage("svcId");
                if (svcId) {
                    const item =
                        this.d4bSvcList.find((item) => item.svcId == svcId) ||
                        this.d4bSvcList[0];
            ...
            Promise.all([
                ...
                basicData.puApiParams.forEach((item, index, paramList) => {
                    ...
                    if (
                        ...
                        // item.paramMappingKey = packageParamKey(
                        //     item,
                        //     basicData.puApiParams
                        // );
        ...
        get4BTabsList() {
            if (
                this.defaultActive == "processDesc" ||
        ...
        topMenuBtnClick(num) {
            if (num == "1") {
                ...
                // S4设计提交
                this.checkS4SystemDesignResult(false);
            } else if (num == "4") {
                // c4开发提交
                this.checkS4SystemDesignResult(true);
            }
        },
        /**
         * 选择接口弹框
         */
        addInterface(val, type) {
            if (type) {
        ...
        async checkS4SystemDesignResult(status) {
            let taskTpCd = "08-02";
            if (status) {
                // 08-02
                taskTpCd = "11-02";
            ...
            const res = this.rpc.assetList.updateFunctionTaskStatus({
                examId: this.$route.query.examId,
                fnctId: this.$route.query.fnctId,
                taskTpCd: taskTpCd,
                taskStsCd: "02",
            });
            this.checkSystemDesign();
    ...
    created() {
        this.itemInfo = {
            ...
            bizAppNo: this.$route.query.appId || this.$route.query.bizAppNo,
            bizAppVersion: "",
