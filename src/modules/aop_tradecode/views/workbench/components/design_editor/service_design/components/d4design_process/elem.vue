      ...
      >
        ...
        >
          ...
          <el-tooltip
                ...
                placement="top"
            >
            <i class="el-icon-plus"  @click="addService" v-if="showAdd" disabled></i>
          </el-tooltip>
                  ...
                  <div class="text_area" @mousedown.stop="pickOne(item, $event, 'baseCpn')">
                    ...
                    <div v-if="!['break组件', 'continue组件', '运算组件'].includes(item.cpntNm)" class="icon">
                      <span
                        class="center"
                        :class="{
                          blue: ['循环组件', '选择组件', '设置会话组件', '获取会话组件', '生成图形验证码', '校验图形验证码', '设置缓存组件', '获取缓存组件', '更新缓存组件', '更新会话组件', '集合处理组件'].includes(item.cpntNm),
