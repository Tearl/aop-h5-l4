<template>
    <div class="aop_tradecode_comps_d4editor_service_define_check_dialog">
            ...
            <div slot="box">
                <div class="standar_check_dialog">
                    <ul class="standar_check_list">
                        <li
                            v-for="(item, index) in standardCheckData"
                            :key="index"
                            class="list_item"
                        >
                            <div class="left">
                                <img
                                    class="test_record_icon"
                                    :src="
                                        item.checkFlg == '-1'
                                            ? item.loadingIcon
                                            : item.checkFlg == '0'
                                ...
                                <template v-if="!item.case">
                                    ...
                                    <span
                                        class="text result"
                                        :class="['success']"
                                        v-if="item.checkFlg == '1'"
                                        >成功</span
                                    >
                                </template>

                                <template v-else>
                                    <span class="text result_tips">{{
                                        item.tips
                                    }}</span>
                                </template>
                            </div>
                            <div class="right">
                                ...
                                <el-button
                                    type="primary"
                                    size="mini"
                                    :disabled="item.checkFlg != '0'"
                                    @click="showCheckDetail(item.id)"
                                    >详情
                                </el-button>
                            </div>
                        </li>
                    ...
                    <div v-show="successFlag && checkEnd" class="success_box">
                        <img
                            class="test_record_icon"
                            :src="require('@m/assets/images/icon_success.png')"
                        />
                        <span>校验通过</span>
                    </div>
                    <div v-show="!successFlag && checkEnd" class="success_box">
                        <img
                            class="test_record_icon"
                            :src="require('@m/assets/images/icon_fail.png')"
                        />
                        <span>校验失败</span>
                    </div>
                </div>
        ...
        <PageDialog
            :dialogWidth="'720px'"
            :dialogTitle="'详情'"
            :dialogVisiable="standardCheckDetailVisible"
            @closeDialog="standardCheckDetailVisible = false"
            class="detail_dialog"
        >
            <div slot="box">
                ...
                <div v-if="currentShowTitle == '4'">
                    ...
                    <Table
                        class="table"
                        :data="tranParamUsedResultList"
                        :table="noStandarParamsColumn"
                    ></Table>
                </div>
                ...
                <div v-if="currentShowTitle == '7'">
                    <div class="title">
                        已设置来源值接口输出参数在来源方不存在列表：
                    </div>
                    <Table
                        class="table"
                        :data="sourceParamList"
                        :table="noStandarParamsColumn"
                    ></Table>
                </div>
                <div v-if="currentShowTitle == '8'">
                    ...
                    <Table
                        class="table"
                        :data="actionParamUsedResultList"
                        :table="actionColumn"
                        @operate="operate"
                    ></Table>
                </div>
                ...
                <div v-if="currentShowTitle == '10'">
                    ...
                    <Table
                        class="table"
                        :data="l5ParamList"
                        :table="nol5serviceList"
                    ></Table>
                </div>
                <div v-if="currentShowTitle == '11'">
                    <span class="title"
                        >已设置来源值组件输入参数在来源方不存在列表</span
                    >
                    <Table
                        class="table"
                        :data="tranParamSourceResultList"
                        :table="actionColumn"
                        @operate="operate"
                    ></Table>
                </div>
            ...
            <div slot="footer">
                <el-button
                    type="primary"
                    @click="standardCheckDetailVisible = false"
                    >确 定</el-button
                >
            </div>
        </PageDialog>
        ...
        >
            <div slot="box">
                ...
                <Table
                    class="table"
                    :data="compsParamsList"
                    :table="noStandarParamsColumn"
                ></Table>
            </div>
            <div slot="footer">
                <el-button type="primary" @click="actionDetailVisible = false"
                    >确 定</el-button
                >
            </div>
        </PageDialog>
    ...
    methods: {
        ...
        async showStandardCheckDialog(data) {
            ...
            await setTimeout(() => {
                ...
                this.standardCheckData[4].checkFlg =
                    data.txInputParamUsedInLayoutCheck;
                this.standardCheckData = [...this.standardCheckData];
            }, 3000);
            await setTimeout(() => {
                // 接口输出参数未被完全赋值检测
                this.setParamList = data.txOutputParamBeAssignResultList;
                this.standardCheckData[5].checkFlg =
                    data.txOutputParamBeAssignCheck;
                console.log(
                    data.txOutputParamBeAssignCheck,
                    "txOutputParamBeAssignCheck"
                );
                this.standardCheckData = [...this.standardCheckData];
            }, 3500);
            await setTimeout(() => {
                ...
                this.standardCheckData[6].checkFlg =
                    data.txOutputParamSrcIdCheck;
                this.standardCheckData = [...this.standardCheckData];
            }, 4000);

            await setTimeout(() => {
                // 组件输入参数未被完全赋值检测
                this.actionParamUsedResultList =
                    data.actionIsNeedInputParamBeAssginResultList;
                this.standardCheckData[7].checkFlg =
                    data.actionIsNeedInputParamBeAssginCheck;
                this.standardCheckData = [...this.standardCheckData];
            }, 4500);
            await setTimeout(() => {
                // 组件输出参数引用检测
                this.paramCheckList = data.actionOutputParamBeUserOneResultList;
                this.standardCheckData[8].checkFlg =
                    data.actionOutputParamBeUserOneCheck;
                this.standardCheckData = [...this.standardCheckData];
            }, 5000);
            await setTimeout(() => {
                // L5服务未被完全编排检测
                this.l5ParamList = data.l5serviceBeUserInLayoutResultList;
                this.standardCheckData[9].checkFlg =
                    data.l5serviceBeUserInLayoutCheck;
                this.standardCheckData = [...this.standardCheckData];
            }, 5500);
            await setTimeout(() => {
                // 组件输入参数来源检测
                this.tranParamSourceResultList =
                    data.actionOutputParamSrcIdResultList;
                this.standardCheckData[10].checkFlg =
                    data.actionOutputParamSrcIdCheck;
                this.checkEnd = true;
                let flag = this.standardCheckData.filter(
                    (item) => item.checkFlg != "1" && (item.id != '5' && item.id != '9')
                );
