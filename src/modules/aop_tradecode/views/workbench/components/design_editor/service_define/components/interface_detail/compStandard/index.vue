            ...
            <div class="table-header">
              ...
              <span class="current-font marginleft">
                当前字段简称：<span class="font-content">{{
                  fieldInfo.paramKey || fieldInfo.fieldId
                }}</span></span
              >
...
export default {
  ...
  methods: {
    ...
    getTableData() {
      ...
      this.rpc.d4
        .qryDataDictionaryFieldRecommendList({
          fieldKeyName: this.fieldInfo.paramKey || this.fieldInfo.fieldId,
        })
        .then((res) => {
    ...
    async handleConfirm() {
      await this.rpc.d4.AddParamMapping({
        paramId: this.fieldInfo.paramId,
        mappingType: this.$route.query.mappingType || "0",
        svcId:this.$route.query.apiId,
        paramKey:this.fieldInfo.paramKey || this.fieldInfo.fieldId,
        mappingFieldId:
