...
export default {
  ...
  methods: {
    ...
    saveParams() {
      this.$emit("saveParams", this.tableData, this.formInline);
    },
    // tableData数据选中表格中
    isTableData() {
    ...
    showDialog(e, paramDirection) {
      console.log("data", e, paramDirection)
    ...
    //字段
    async getFiledList() {
      Promise.all([this.rpc.d4.getL5ServField({
        ...
        this.dictList = this.dictList.filter((item) => item.paramDirection == paramDirection);
    ...
    setFiledSourceAndSourceValue(paramList, allParamList) {
      paramList.forEach(item => {
        ...
        const _item = allParamList.find(param => param.paramCName == item.paramCName && param.paramKey == item.paramKey && param.paramDirection == "1");
        item.paramMappingKey = _item ? packageParamKey(_item, allParamList) : ""
        item.apSource = "20"
