  ...
  methods: {
    ...
    async getSourceList() {
      ...
      // return res.apiInfoList;
          ...
          //   temp = false
          // }
          // this.setDisabled(item.children, temp)
    ...
    async updateCompositeIdMapToParamList(row) {
      ...
      if (compositeId && item && !(item?.paramList?.length)) {
        ...
        } else if (["uploadAction", "getCacheAction", "setCacheAction", "assignAction",].includes(item.actionType)) {
          // 'autoAction',  
          // 'signCustAddAction',
          // 'setAcctLimitAction',
          // 'setRoleAction',
