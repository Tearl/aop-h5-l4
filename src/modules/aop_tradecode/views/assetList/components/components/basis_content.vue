...
export default {
    ...
    methods: {
        ...
        checkList(e) {
            console.log(e)
            console.log(this.leftCheckList,'已选列表')
            e.forEach((ele,index)=>{
                console.log(ele.d4iId)
                this.leftCheckList.forEach((obj)=>{
                ...
                this.oldleftCheckList.forEach((item)=>{
    ...
    watch: {
        "tableA": {
            handler(newVal, oldVal) {
                // console.log("点击要素查询交互列表更新",newVal)
                if(!newVal){return}
                this.$nextTick(()=>{
        ...
        "removeleftCheckList": { //contentShow
            handler(newVal, oldVal) {
                if(!newVal){return}
