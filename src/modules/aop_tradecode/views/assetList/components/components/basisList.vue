<template>
    <div class="aop_developer_serviceList">
        <div v-if="tabType == 'A'" class="preview_content_table">
                    ...
                    <div class="select_block">
                        ...
                        <div class="tree_block" :style="{ height: mainHight }" @scroll="beforeScroll">
                            ...
                            >
                                ...
                                >
                                    <span class="icon-left">素</span>
                                    <span slot="title">{{ item.elementName }}</span>
                                </el-menu-item>
                            </el-menu>
                        </div>
                    </div>
...
import ListTableA from "./basis_content.vue";
...
export default {
    ...
    methods: {
        ...
        async getElementList(row) {
            ...
            let res = await this.rpc.assetList.getElementList(params);
            if (Array.isArray(res.elementInfos) && res.elementInfos.length) {
                const list1 = res.elementInfos.map((i,index) => {
                    return {
                        id: index + 1,
                        itemIndex: '1-' + '' + (index +1),
                        isClick: false,
                        modelVersion: i.modelVersion,
                        isolateWorkspace: i.isolateWorkspace,
                        elementType: i.elementType,
                        elementName: i.elementName,
                        elementId: i.elementId,
                        elementDesc: i.elementDesc,
    ...
    watch: {
        filterText(val) {
            if(val){
                this.treeData = this.treeData.filter((item)=>item.elementName.indexOf(val) != -1);
            }else{
