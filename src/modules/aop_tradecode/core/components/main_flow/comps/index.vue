<template>
  <div class="aop_tradecode_comps_process_decision">
    <div ref="svgBox" class="process_svg_box">
      ...
      >
        ...
        >
          ...
          <!-- 输入和输出 -->
          <rect
            v-if=" ['inputAction', 'outputAction'].includes(item.actionType)
            "
            :x="item.actionX"
            :y="item.actionY"
            :width="getItemWidth(item)"
          ...
          >
            <div class="process_rect_text flex h100 ellipsis">
          ...
          <!-- 判断组件文字内容 -->
          <foreignObject
            v-if="['judgeAction'].includes(item.actionType)"
            :x="item.actionX"
            :y="item.actionY"
            :width="getItemWidth(item)"
            :height="getItemHeight(item)"
          ...
          >
            <div class="judge_text_wrapper ellipsis judge_text_white_wrapper">
              <div class="content bottom">{{ `${getJudgeCondition(item)}` }}</div>
            </div>
          </foreignObject>

          <!-- 新增、删除、查询、修改、异常组件文字内容 -->
          <foreignObject
            v-if="['addAction', 'deleteAction', 'queryAction', 'modifyAction', 'exceptionAction'].includes(item.actionType)"
            :x="item.actionX"
            :y="item.actionY"
            :width="getItemWidth(item)"
            :height="getItemHeight(item)"
            :style="{ textAlign: 'center' }"
          >
            <div class="judge_text_wrapper ellipsis">
              <div class="content bottom">{{ getItemDisplayText(item) }}</div>
            </div>
          </foreignObject>
          ...
          <foreignObject
            v-if="['cacheSessionAction', 'getSessionAction', 'createGraphValidateCodeAction', 'verifyGraphValidateCodeAction', 'getCacheAction', 'setCacheAction', 'updateCacheAction', 'updateSessionAction', 'setHeaderAction',].includes(item.actionType)"
            :x="item.actionX"
            :y="item.actionY"
            :width="getItemWidth(item)"
            :height="getItemHeight(item)"
          ...
          >
            ...
            <p class="d_rect_code" :class="{
              blue: ['cacheSessionAction', 'getSessionAction', 'createGraphValidateCodeAction', 'verifyGraphValidateCodeAction', 'getCacheAction', 'setCacheAction', 'updateCacheAction', 'updateSessionAction', 'setHeaderAction',].includes(item.actionType)
            }">
              {{ getDServText(item, "actionDesc") }}
            </p>
            <p class="d_rect_msg mt5">
    ...
    */
      ...
      for (let i in me.dataList) {
        ...
        for (let p in me.lineList) {
          if (me.lineList[p].sourceId.split("-")[0] == me.dataList[i].actionId) {
            ...
            d.lineType = me.lineList[p].lineType
            children.push(d);
          }
        }
    ...
    upDateServParams(obj, e) {
      ...
      if (["D4bAction", "D5SelfAction", "D5LinkAction", "decisionAction"].includes(obj.actionType)) { // 服务组件
        ...
        target.compositeId = e.compositeId
        target.actionName = e.confList.find(item => item.confKey == "actionDesc")?.confValue
      } else if (['addAction', 'deleteAction', 'queryAction', 'modifyAction', 'exceptionAction'].includes(obj.actionType)) {
        const target = this.dataList.find(item => item.actionId == obj.actionId)
        // target.paramList = e.paramList
        target.conf = e.confList.map(item => {
          return {
      ...
      for (let i in e) {
        ...
        for (let k in e[i].transition) {
          let n = {
            ...
            lineConditionDesc: e[i].transition[k].lineConditionDesc
          };
          ll.push(n);
        }
      }
    ...
    getLineText(lineObj, showAll = false) {
      if (lineObj.sourceId && lineObj.sourceId.indexOf("judgeAction") > -1) {
        return lineObj.lineConditionDesc
        // try {
      ...
      } else if (lineObj?.sourceId?.includes("customAction")) {
        const sourceItem = this.dataList.find(data => data.actionId == lineObj.targetId)
        const text = this.getConfItem("actionDesc", sourceItem.actionId)?.value || ""
        return text
      } else {
        return ""
      }
    ...
    // 获取判断组件的显示内容
    getJudgeCondition(item) {
      const expressionList = JSON.parse(this.getConfItem("expressionList", item.actionId)?.value)
      // console.log("getJudgeCondition", expressionList)
      const str = expressionList.reduce((prev, item) => {
        const left = item.leftFormSource == "20" ? item.leftFormParaMapgNm : item.leftFormParamMappingKey
        const condTyp = item.condTyp
        const right = item.rightFormSource == "20" ? item.rightFormParaMapgNm : item.rightFormParamMappingKey
        prev = `${prev}【${left}】${condTyp}${right}`
        return prev
      }, "")
      return str || item.actionName
    }
  },
  watch: {
    dataObj(n, o) {
...
.aop_tradecode_comps_process_decision {
  position: relative;
  .process_svg_box {
    width: 100%;
    height: 100%;
