...
export default {
  ...
  methods: {
    ...
    getItemDisplayText(item) {
      ...
      } else if (['addAction', 'deleteAction', 'queryAction', 'modifyAction', 'exceptionAction'].includes(item.actionType)) {
        const text = this.getConfItem("actionMsg", item.actionId)?.value
        return text || item.actionName
      } else {
          ...
          children.forEach((childInfo, index, arr) => {
            ...
            if (child) {
              ...
              if (childInfo.lineType == "polyline") {
                ...
                lineObj.lineConditionDesc = childInfo.lineConditionDesc
                if (isTest) {
