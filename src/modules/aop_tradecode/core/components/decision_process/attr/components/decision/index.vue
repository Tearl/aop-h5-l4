      ...
      <div class="attr_box">
        <div class="base_info">
          ...
          <ul class="base_info_list">
            ...
            <li class="item">
              <span class="title">规则ID：</span>
              <span class="content">{{ formData.svcId }}</span>
            </li>
            <li class="item">
              <span class="title">规则类型：</span>
  ...
  methods: {
    ...
    async getServiceCpnParams(e) {
      ...
      if (e.compositeId) {
        ...
        Promise.all([
          this.rpc.transactionDesign.getServiceCpnParams(params),
          this.rpc.transactionDesign.queryDecisionTreeParamAndRuleDefinitions(params),
          this.rpc.transactionDesign.getDecisionDetail({
            apiId: e.actionOwnId,
          }),
        ]).then(([res, ruleInfo, decisionInfo]) => {
          ...
          this.dataObj = JSON.parse(JSON.stringify(decisionInfo))
        });
      } else {
    ...
    async getServiceCpnDefaultParams(e) {
      ...
      Promise.all([
        this.rpc.transactionDesign.queryDecisionTreeParamAndRuleDefinitions(
          params
        ),
        this.rpc.transactionDesign.getDecisionDetail({
          apiId: e.actionOwnId,
        }),
      ]).then(([res, decisionInfo]) => {
        this.formData = res
