<template>
  <div class="aop_tradecode_comps_design_attr_create_graph_validate_code_attr">
    <div class="attr_main">
      ...
      <div v-if="!isRead && !fourRAttrFlag" class="attr_box">
        <div class="attr_show">
          ...
          <div class="attr_show_item">
            ...
            <div class="attr_show_item_c">
              <el-input v-model="formData.codeLength" size="mini" placeholder="验证码位数"></el-input>
            </div>
          </div>
          <div class="attr_show_item">
            <div class="attr_show_item_t required">
          ...
          <div class="attr_show_item">
            ...
            <div class="attr_show_item_c">
              <el-select
                :disabled="!canSave"
                v-model="formData.cacheStrategy"
                size="mini"
              >
                <el-option
                  v-for="item in cacheStrategyOptions"
                  :key="item"
                  :label="item.label"
                  :value="item.value"
                ></el-option>
          ...
          <div class="attr_show_item">
            ...
            <div class="attr_show_item_c">
              <span class="text_click" @click="showDialog('responsePicture')">
  ...
  methods: {
    async getParams(e, obj) {
      ...
      if (!e.compositeId) {
        ...
        // if (this.formData.cacheStrategy == "") this.formData.cacheStrategy = this.currentServObj?.svcId
    ...
    async saveParams() {
      ...
      } else if (!this.formData.codeLength) {
        return this.$message.error("验证码位数不能为空")
      } else if (!/^[0-9]*$/.test(this.formData.codeLength)) {
        return this.$message.error("验证码位数只能输入数字")
      } else if (!this.formData.codeType) {
        return this.$message.error("验证码类型不能为空")
      } else if (!this.formData.cacheStrategy) {
        return this.$message.error("缓存策略不能为空")
      } else if (!this.formData.responsePicture) {
        return this.$message.error("返回图片不能为空")
      } else if (!this.formData.uniqueKey) {
        return this.$message.error("返回缓存key不能为空")
      } 
      ...
      const _formData = JSON.parse(JSON.stringify(this.formData))
      // _formData.cacheStrategy = _formData.cacheStrategy == this.currentServObj?.svcId ? "" : _formData.cacheStrategy
    ...
    getDisplayText(type) {
      if (type == "codeType") {
        const option = this.codeTypeOptions.find(item => item.value == this.formData.codeType)
        return option?.lable || ""
      } else if (type == "cacheStrategy") {
        const option = this.cacheStrategyOptions.find(item => item.value == this.formData.cacheStrategy)
        return option?.lable || ""
      } else if (type == "responsePicture") {
        const responsePicture = JSON.parse(this.formData.responsePicture || "{}")
        return responsePicture?.paramNm || "选择"
...
.aop_tradecode_comps_design_attr_create_graph_validate_code_attr {
  position: relative;
  height: 100%;
  .mt10 {
    margin-top: 10px;
