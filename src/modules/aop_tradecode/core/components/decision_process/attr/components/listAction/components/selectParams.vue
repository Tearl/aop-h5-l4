...
export default {
  ...
  computed: {
    options() {
      return this.innerFlowDataList.reduce((prev, data) => {
  ...
  methods: {
    ...
    async selectChange(e) {
      ...
      this.dictionaryData = this.dictionaryData.filter((item) => {
        return item.paramType === 'List'
      })
