      ...
      >
        ...
        >
          ...
          <el-input
            ...
            :class="
              item.disabled ? 'aop_workgate_comps_page_table_down_disabled' : ''
            "
            :placeholder="item.placeholder"
            @change="inputChange(scope.row)"
            @input="(e) => item.input && item.input(e, scope)"
          ...
          ></el-switch>
          <div v-else-if="item.type == 'inputText'">
            <el-input
              v-if="scope.row.elementType === 'input'"
              :style="item.inputStyle"
              v-model="scope.row[getProp(item, scope)]"
              :placeholder="item.placeholder"
              :clearable="item.clearable || true"
...
export default {
  ...
  methods: {
    ...
    getSpanArr(data) {
      ...
      console.log("this.pos", this.spanArr);
    },
    objectSpanMethod({ row, column, rowIndex, columnIndex }) {
      if (this.mergeColumnList.includes(columnIndex)) {
        const _row = this.spanArr[rowIndex];
        console.log(this.spanArr, "row rowIndex", rowIndex);
    ...
    getProp(item, scope) {
      return typeof item.type == "string" ? item.prop : item.prop(scope.row);
    },
    getType(item, scope) {
...
.aop_workgate_comps_page_table {
  ...
  .aop_workgate_comps_page_table_down_disabled {
    color: red;
