...
- prop:         表格prop属性
- filter:       过滤器方法名
- width:        column 最小宽度 （可传可不传） 
- type:         显示类型 input,select,text,enum,switch, button, checkbox(有true与false两个属性对应true-label，false-label), textClick(文字可点击触发事件,有默认样式),textClassList(通过表格的属性值判断文字样式),radio, inputPicker(输入框或者时间选择器，通过inputPicker方法去判断)
...
- clearable:    输入框或者下拉框是否有清空按钮，如果没有则传"false",有可不传可传"true"
...
- 设置了fixed的table需要添加tooltip: true 不让换行
