...
export default {
  ...
  methods: {
    ...
    confirm() {
      ...
      this.setParamListSource(selectedTable, paramList, this.formData.actionId, actionName, "00")
      // const paramList = this.selectedTable.map(param => {
    ...
    async selectChange(e) {
      ...
      this.dictionaryData.forEach(param => {
        // param.paramKey = packageParamKey(param, paramList)
        console.log(param.paramKey, 'param')
        param.value = param.paramKey
        if (param.children && param.children.length) {
          this.deepChildren(param.children, paramList)
        }
      })
      console.log(this.dictionaryData, "dictionaryData")
    ...
    getDataDealOutputParmas(e) {
      ...
      this.dictionaryData = e.paramList.filter((item) => item.paramDirection == "0")
    },
    async getServiceCpnDefaultParams(e) {
      ...
      this.dictionaryData = res.paramList.filter((item) => item.paramDirection == "0")
    },
