...
export default {
  ...
  inject: ["getDataList"],
  methods: {
    async showDialog(data, formData, key="actionId", cptData={}) {
      ...
      console.log(this.currentRadio, 'this.currentRadio=======')
      ...
      // }
      await this.updateCompositeIdMapToParamList(key, cptData)
      console.log(data[key], key)
      const value = data[key]
      const item = this.compositeIdMapToParamList[value]
      let paramList = JSON.parse(JSON.stringify(this.compositeIdMapToParamList[value].paramList))
      const paramDirection = value == this.currentServObj?.svcId ? "0" : "1"
      ...
      paramList.forEach(param => {
        ...
        //   console.log('p[pppp]')
        //   this.deepChildren(param.children, paramList)
        // }
      })
      paramList.forEach(param => {
        param.paramKey = param.value
      })
      this.dictionaryData = reList(paramList)
    },
    deepChildren(children, paramList) {
    ...
    async updateCompositeIdMapToParamList(key, cptData) {
      console.log(cptData, 'cptData----')
      const compositeId = this.currentRow[key]
      // 新拖进来的组件
      ...
      if (compositeId && item && !(item?.paramList?.length)) {
        if (apiId && ["D4bAction", "D5SelfAction", "D5LinkAction"].includes(item.actionType)) {
          ...
          // })
          const res = cptData
          item.paramList = reList(res.paramList)
        } else if (item.actionType == "loopAction" || item.actionType == "getCacheAction" || item.actionType == 'getFlowNumAction') {
        ...
        } else if (item.actionType == "uploadAction") {
          // const res = await this.rpc.d4.getServiceCpnParams({
          //   d4StusCd: "1",
          //   blngtoTyp: "11",
          //   compositeId: compositeId,
    ...
    tableCellStyle({ row, column, rowIndex, columnIndex }) {
      if(column.type === "selection") {
        ...
        if (item && ["cacheSessionAction", "getSessionAction", "uploadAction", 'updateSessionAction', 'getCacheAction', 'getFlowNumAction'].includes(item.actionType)) {
