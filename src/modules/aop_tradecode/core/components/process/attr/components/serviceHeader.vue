...
export default {
          ...
          },
          {
            ...
            filter: (scope) => {
              ...
              if (actionId == 'constantValue') {
                ...
                return ""
              } else if (["当前4b交易", "当前循环组件", "枚举值", '复合来源'].includes(actionName)) {
                return actionName
              } else {
  ...
  methods: {
   ...
   async handleSelectChange(e) {
    ...
    saveServiceCpnParams() {
      if (!this.confFormData.svcId) {
        this.$message.error("请选择服务来源")
        return
      }
      ...
      for (let i = 0; i < paramList.length; i++) {
        ...
        if (item.actionId !== 'complex' && item.paramOrder[0] !== '2') {
          if (item.actionName && !item.paramMappingKey) {
            console.log(item, "item.actionName")
      ...
      //   let paramKeySrcType = ''
      //   if (item.actionId === this.currentObj.svcId) {
      //     paramKeySrcType = '00'
      //   } else {
      //     paramKeySrcType = '20'
      //   }
      ...
      //   } else if (id === this.currentObj.svcId) {
      ...
      if (e.compositeId) {
        ...
        // ]).then(([res]) => {
          ...
          for (let i = 0; i < this.options.length; i++) {
            ...
            if (item.value === this.confFormData.svcId) {
              return
            }
          }
          this.confFormData.svcId = ''
    ...
    async getServiceCpnDefaultParams(e) {
      ...
      this.confKeyList.forEach((item) => {
        ...
        this.confFormData[item.key] = target?.value || "";
      })
