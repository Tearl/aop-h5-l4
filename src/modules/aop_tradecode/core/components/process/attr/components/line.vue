<template>
  <div class="aop_tradecode_comps_design_attr_line">
    <PageDialog
      :dialogTitle="'条件配置'"
...
import SelectTypeDialog from "./components/judge/select_type";
...
export default {
  ...
  computed: {
    ...
    paramCNameTree() {
      if (!this.currentServObj?.puApiParams) return []
  ...
  methods: {
    ...
    showDialog(type, data, judgeCpn) {
      this.type = type;
      if (type == "add") {
        // 新增
        this.dataList = this.getDataList();
        console.log("新增条件", type, data, judgeCpn);
        const triggerCpnName = this.dataList.find(
          (item) => item.actionId == data.targetId
        )?.actionName;
        this.formData.triggerCpnName = triggerCpnName;
        this.formData.sourceId = data.sourceId;
        this.formData.targetId = data.targetId;
        this.lineObj = data;
        this.currentObj = judgeCpn;
        this.dialogVisible = true;
      } else if (type == "edit") {
        // 修改
        console.log("修改条件", type, data, judgeCpn);
...
.aop_tradecode_comps_design_attr_line {
  .edit_form {
    margin-top: 10px;
    @include form;
    .ap_soure_50 {
