...
export default {
  ...
  methods: {
    ...
    confirm2(data) {
      if (data?.length) {
        this.formData.loopList = data[0].paramMappingKey
        this.formData.loopListSource = data[0].actionId
        const paramList = mixList(setParamOrder(data), "children")
        console.log(data, 'datadata', this.tableData[0])
    ...
    async saveParams() {
      ...
      const allList = mixList(this.tableData, "children")
    ...
    showDialog2(key) {
      this.$refs.selectParamsRef2.showDialog(key);
    },
    confirm(key, data) {
    ...
    init() {
      ...
      puApiParams = puApiParams.filter((item) => item.paramDirection == '1')
