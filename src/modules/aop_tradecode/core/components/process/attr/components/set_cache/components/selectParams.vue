...
export default {
  ...
  computed: {
    options() {
      let list = this.getDataList().reduce((prev, data) => {
  ...
  methods: {
    ...
    tableCellStyle({ row, column, rowIndex, columnIndex }) {
      ...
      if(column.type === "selection") {
        ...
        // console.log("_parentParamtableCellStyle", _parentParam, topParamParent, row, column)
        // console.log(["List", "Object"].includes(_parentParam?.paramType) && _parentParam?.paramKey != "bodyOutData", topParamParent?.paramKey != "bodyOutData", row?.paramKey == "bodyOutData", column)
        if ((_parentParam?.paramType.includes('List') || _parentParam?.paramType.includes('Object')) && _parentParam.paramKey === 'bodyData') {
          return
        }
