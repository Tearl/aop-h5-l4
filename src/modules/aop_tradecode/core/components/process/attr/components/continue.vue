<template>
  <div class="aop_tradecode_comps_design_attr_continue_attr">
    <div class="attr_main">
  ...
  methods: {
    async getParams(e, obj) {
      ...
      if (!e.compositeId || this.fourRAttrFlag) {
        ...
        this.tableData = reList(e.paramList.filter((item) => item.paramDirection == 0));
      }
...
.aop_tradecode_comps_design_attr_continue_attr {
  position: relative;
  height: 100%;
  .attr_main {
    height: 100%;
