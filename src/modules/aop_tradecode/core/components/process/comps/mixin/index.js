...
export default {
  data() {
    return {
      actionTypeToName: {
        ...
        'desensitizationAction': '脱敏组件',
        'listSetValAction': '集合赋值组件',
        'captchaAction': '校验验证码组件',
        'getDesensitizationAction': '获取脱敏数据组件',
        'timeAction': '时间组件',
        'keyMapAction': '字段映射组件',
  ...
  methods: {
    ...
    getItemHeight(item = {}) {
      ...
      } else if (["throwExceptionAction", "transAction", "domainAction", "systemAction", "newD5Action", "D4bAction", "D5SelfAction", "D5LinkAction", "cacheSessionAction", "getSessionAction", "createGraphValidateCodeAction", "verifyGraphValidateCodeAction", "getCacheAction", "setCacheAction", 'updateCacheAction', 'updateSessionAction', 'listAction', 'setHeaderAction', 'transferStateAction', 'listSetValAction', 'captchaAction', 'desensitizationAction', 'getDesensitizationAction', 'timeAction', 'keyMapAction', 'spliceAction', 'getFlowNumAction',].includes(item.actionType)) {
    ...
    */
    ...
    getLineList(lineList, dataList, itemWidth, itemHeight, isTest) {
      let _origin_linelist = JSON.parse(JSON.stringify(lineList));
      lineList.length = 0
          ...
          children.forEach((childInfo, index, arr) => {
            ...
            if (child) {
              ...
              if (childInfo.lineType == "polyline") {
                ...
                lineObj.relId = childInfo.relId
                lineObj.isTfrCdtnCd = _origin_linelist.filter(
                    (i) => i.lineId == lineObj.lineId
                ).length ? _origin_linelist.filter(
                    (i) => i.lineId == lineObj.lineId
                )[0].isTfrCdtnCd : false;
                lineObj.lineCondition = _origin_linelist.filter(
                    (i) => i.lineId == lineObj.lineId
                ).length
                    ? _origin_linelist.filter(
                          (i) => i.lineId == lineObj.lineId
                      )[0].lineCondition
                    : null;
                if (isTest) {
                ...
                if (child.actionType == "sbsectAction") {
                  ...
                  lineObj.isTfrCdtnCd =  _origin_linelist.filter((i) => i.lineId == lineObj.lineId)
                      .length
                      ? _origin_linelist.filter(
                            (i) => i.lineId == lineObj.lineId
                        )[0].isTfrCdtnCd
                  ...
                  // console.log("getLineList", this.getChild(childInfo, dataList), x2, y2)
