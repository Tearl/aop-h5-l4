<template>
  <div class="aop_tradecode_comps_dicmgmt_dic_form">
    ...
    <DictionarySelect
      ref="dictionaryRef"
      :sysId="sysId"
      @addChildParams="setDictChildList"
    >
    </DictionarySelect>
  </div>
</template>
...
export default {
  ...
  computed: {
    isShowEnum() {
      return [
        ...
        'Short',
        'Long',
      ].includes(this.formData.dictryTyp)
    },
    isDisabled() {
    ...
    selectTypeList() {
      if (this.formData.attrType === '4') {
        return this.selectVal.filter((e) => {
          if (e.type !== 'Object' && e.type !== 'List' && e.type !== 'File') {
            return e
          }
        })
      } else {
  ...
  methods: {
    radioChange() {
      if (this.formData.dictryTyp === 'List' || this.formData.dictryTyp === 'Object' || this.formData.dictryTyp === 'File') {
        if (this.formData.attrType === '4') {
    ...
    async validator (rule, value, callback) {
      if (rule.field == "dictryLength") {
        ...
        if (this.formData.dictryNo) {
          ...
          console.log("中文分类",params,this.sysId)
    ...
    async getRuleList() {
      ...
      const res = await this.rpc.publicEditor.getDataRuleList(data);
    ...
    async addDictionary () {
      ...
      const res = await this.rpc.publicEditor[api](params)
      this.$notify({
        title: "成功",
        message: "成功",
        duration: 2000,
    ...
    handleCancel () {
      if (!this.dictryId) {
        if (this.type !== "public") {
          ...
          this.$router.push({
            path: "/aop_tradecode/dicmgmt/list",
          })
        }
    ...
    // 点击确定按钮
    async preservation () {
      const res = await this.$refs.formData.validate()
      if (!res) {
        this.$message.error("基本信息填写有误或未填写")
        return
      }
      const params = await this.addDictionary()
      return params
    },
    // 进入编辑页面时查询详情
    async getDictionaryInfoById (e, operation) {
      ...
      this.formData.ruleId = res.ruleList.length ? res.ruleList[0].ruleId : ''
    ...
    init () {
      ...
      this.beltLine = this.$route.query.beltLine || getSessionStorage('beltLine');
...
.aop_tradecode_comps_dicmgmt_dic_form {
