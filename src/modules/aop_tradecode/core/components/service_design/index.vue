<template>
    <div class="aop_tradecode_page_d4workbench_workbench_service_design">
        ...
        <AsNav @navBack="navBack" :asideActive="asideActive">
            <div slot="textProps">功能设计开发-{{ fnctNm }}</div>
        ...
        <div class="design_container">
            ...
            <div class="design_container_process">
                <!-- 流程图 -->
                <div
                    class="process_contain_box"
                    v-show="navDefaultActive == 's'"
                >
                    <div class="process_container">
                        ...
                        >
                            <InterfaceElem
                                ...
                                :disabled="
                                    navDefaultActive == 's' && fourRAttrFlag
                                "
                                ...
                                :isRead="pageMode == 'read' || isDisabled"
                        ...
                        <div class="process_middle">
                            ...
                            >
                                <div class="left-btns">
                                    <div class="btn-item" v-if="picDefault == '4bFlow'">
                                        <el-tooltip
                                            class="item"
                                            effect="dark"
                                            content="一键对齐"
                                    ...
                                    <div class="btn-item" v-if="picDefault == '4bFlow'">
                                        ...
                                        >
                                            <img
                                                v-if="process_type == '02'"
                                                @click="processTypeChange('01')"
                                                src="./imgs/change1.png"
                                            />
                                            <img
                                                v-if="process_type == '01'"
                                                src="./imgs/change2.png"
                                                @click="processTypeChange('02')"
                                            />
                                        </el-tooltip>
                                    </div>
                                    <div class="btn-item">
                            ...
                            <div class="process_box">
                                <div v-if="picDefault == '4bFlow'">
                                    <Process
                                        ...
                                        @showServInfo="showServInfo"
                                        :isRead="isRead || isDisabled"
                                        @clickConditionLine="clickConditionLine"
                                        @toSave="toSave"
                                    >
                                    </Process>
                                </div>
                ...
                <!-- 参数设置 -->
                <div
                    class="params_contain_box"
                    v-show="navDefaultActive == 'p'"
                >
                    <Params
                        ref="paramsRef"
                        :currentServObj="currentServObj"
...
import DemandServList from "./components/dialog/demandServList";
export default {
    mixins: [mixin],
    components: {
        Pattr,
    ...
    data() {
        return {
            ...
            lineLayoutInfo: {},
            interfaceProcessActionInfoList: []
        };
    },
    computed: {
        isDisabled() {
                            ...
                            conf: [
                                {
                                    name: "actionDesc",
                                    value: `${
                                        this.currentServObj.apiName || "input"
                                    }`,
                                },
                                {
                                    name: "actionMsg",
                            ...
                            conf: [
                                {
                                    name: "actionDesc",
                                    value: `${
                                        this.currentServObj.apiName + "结束" ||
                                        "output"
                                    }`,
                                },
                                {
    ...
    provide() {
        return {
            ...
            deleteLineBySourceIdAndTargetId:
                this.deleteLineBySourceIdAndTargetId,
        };
    },
    methods: {
        clickConditionLine(line) {
            console.log('点击条件分支', line);
            this.$refs.pattr.changeShowTypeCondition(line);
        },
        handleClose1(obj) {
        ...
        async getSaveCodeList(row) {
            ...
            let res = await this.rpc.assetList.queryApplicationSaveCodeList(
                params
            );
            this.tranData = [];
        ...
        async downDialog(row) {
            ...
            let res = await this.rpc.assetList.queryAppyBranchCodeDownLoad(
                params
            );
            if (!res.downUrl) {
                this.$message({
        ...
        async getTranList() {
            ...
            let res = await this.rpc.assetList.queryApiInfoListByPageList(
                params
            );
        ...
        async handleActionChange(e) {
            ...
            if (!this.saveStatus) {
                return this.$message.error("请等待保存接口结束");
            }
            ...
            if (changeFlag) {
                this.$confirm(
                    ...
                    .then(async () => {
                        ...
                        this.rpc.newEditor.queryInterfaceProcessLayoutInfo({
                            tranId: this.currentInterfaceObj?.svcId,
                            styleType: this.process_type == '02' ? '00' : '01',
                            parnetActionId: e.compositeId,
                        }).then(result => {
                            const interfaceProcessActionInfoList = result.interfaceProcessActionInfoList || []
                                // 这里做一层数据转换给到getAllData
                            const res = this.flowDataFormat(interfaceProcessActionInfoList)
                            this.getAllData(res);
                        })
                ...
                this.rpc.newEditor.queryInterfaceProcessLayoutInfo({
                    ...
                    const interfaceProcessActionInfoList = result.interfaceProcessActionInfoList || []
                        // this.flowDataFormat = this.flowDataFormat(interfaceProcessActionInfoList)
        ...
        async save(refresh = true, type) {
            let picAddr = null;
            const _file = await this.$refs.process.getPicBySvg(
                this.dataObj.svcSeq
            );
            // 图片文件上送minio
            let fd = new FormData();
            fd.append("uploadFile", _file);
            const { fileInfoLists } = await this.rpc.s4design.processPicUpload(
                fd
            );
            ...
            const dataList = this.getDataList().filter(
                (data) =>
                    data.actionType != "inputAction" &&
                    data.actionType != "outputAction"
            );
            if (!dataList.length)
                return this.$message.error("请添加编排组件后再保存");
            // 给没有位置序号的组件添加一个临时的位置序号
            console.log(dataList, "dataList");
            const noActionSeqItemList = this.getDataList(true).filter(
                (data) => {
            ...
            console.log(this.dataObj, data, "dataObj, data");
            ...
            for (let i = 0; i < flow.length; i++) {
                ...
                const compositeId = element.compositeId;
                if (!keyObj[compositeId]) {
                    keyObj[compositeId] = element;
                    newFlow.push(element);
                }
            }
            ap.flow = newFlow;
            keyObj = {};
            ...
            await this.commitInterfaceProcessLayoutInfo()
            
            await this.rpc.s4design.addFuntionJobCommitFlow({
                fnctId: this.$route.query.fnctId,
                jobTpCd: "C4",
            });
            ...
            const fileActionItem = this.getDataList().find(
                (data) =>
                    data.actionType == "uploadAction" && !data.paramList?.length
            );
        ...
        dataViewFormatFlow() {
            ...
            console.log(l, k, this.lineLayoutInfo,this.process_type, 'dataViewFormatFlow')
            let list = k.map(e => {
                let interfaceProcessActionStyleInfo = {
                    actionId: e.competitorId,
                    styleType: this.process_type == '02' ? '00' : '01',
                    actionWidth: this.$refs.process.itemOptions.itemWidth,
                    actionHeight: this.$refs.process.itemOptions.itemHeight,
                    actionXCoordinate: e.actionX,
                    actionYCoordinate: e.actionY,
                }
                e.interfaceProcessActionLayoutRelList = []

                for(let i = 0; i< l.length; i++) {
                    if (l[i].sourceId.split("-")[0] == e.actionId) {
                        // const child = e.children.find(
                        //     (child) => child.targetId == l[i].targetId
                        // )
                        if(this.lineLayoutInfo.relId && this.lineLayoutInfo.relId == l[i].relId) {
                            if(this.lineLayoutInfo.endActionType == '01') {
                                l[i].condition.exceptionActionInfo.excepInfoJson = this.lineLayoutInfo.exceptionActionInfo.excepInfoJson
                                l[i].condition.executeConditionJson = this.lineLayoutInfo.executeConditionJson
                                l[i].condition.relDesc = this.lineLayoutInfo.relDesc
                                l[i].condition.relDescType = this.lineLayoutInfo.relDescType
                            } else {
                                l[i].condition =  this.lineLayoutInfo
                            }
                            
                        }
                        let m = {
                            ...l[i].condition,
                            interfaceProcessActionLayoutRelStyleInfo: {
                                styleType: this.process_type == '02' ? '00' : '01',
                                styleId: l[i].lineId,
                                relId: l[i].relId,
                                coordinateInfoStr: l[i].polyline
                            },
                            priorityLevel: e.children.findIndex((child) => child.lineId == l[i].lineId) + 1,
                            bjgObjType: this.fourRAttrFlag ? '02' : '01',
                            bjgObjId: this.fourRAttrFlag ? this.currentParentCpnInfo.compositeId : this.currentServObj?.svcId
                        }
                        e.interfaceProcessActionLayoutRelList.push(m)
                ...
                return {
                    ...
                    svcId: e.actionOwnId,
                    parnetActionId: this.fourRAttrFlag ? this.currentParentCpnInfo.compositeId : '',
                    tranId: this.currentInterfaceObj?.svcId,
                    actionCd: e.actionId,
                    actionSeqNo: this.getConfItem(
                                "actionSeq",
                                e.actionId
                            ).value,
                    actionNm: this.getConfItem(
                                "actionDesc",
                                e.actionId
                            ).value,
                    actionType: e.actionType,
                    actionParamList: e.paramList,
                    interfaceProcessActionStyleInfo,
                    interfaceProcessActionConfList: e.conf.map(c => {
                        return {
                            actionId: e.compositeId,
                            confKey: c.name,
                            confValue: c.value,
                    ...
                    interfaceProcessActionLayoutRelList: e.interfaceProcessActionLayoutRelList
                }
            })
            return list
        },
         ...
         * 保存flow最新编排关系
         */
        async commitInterfaceProcessLayoutInfo() {
            const interfaceProcessActionInfoList = this.dataViewFormatFlow()
            const params = {
                tranId: this.currentInterfaceObj?.svcId,
                styleType: this.process_type == '02' ? '00' : '01',
                parnetActionId: this.fourRAttrFlag ? this.currentParentCpnInfo.compositeId : '',
                interfaceProcessActionInfoList: interfaceProcessActionInfoList,
            }
            await this.rpc.d4.commitInterfaceProcessLayoutInfo(params)
            this.lineLayoutInfo = {}
        },
        ...
        async saveServiceArrangeList() {
            ...
            const _layoutActionList = this.layoutActionList.reduce(
                (prev, item) => {
                    prev.push(item);
                    if (item?.layoutActionList?.length) {
            ...
            const compositeIdToLayoutActionList = compositeIdList.reduce(
                (prev, id) => {
                    prev[id] = _layoutActionList.filter(
            ...
            if (this.layoutActionList.length) {
                ...
                let layoutActionList = arrangeDataList.reduce((prev, data) => {
                    let layoutDatas =
                        compositeIdToLayoutActionList[data.compositeId];
                    if (layoutDatas.length) {
                                ...
                                const subLayoutActionList = subDataList.reduce(
                                    (_prev, subData) => {
                                        let subLayoutActions =
                                            compositeIdToLayoutActionList[
                                                subData.compositeId
                                            ];
                                        subLayoutActions =
                                            subLayoutActions.length
                                                ? subLayoutActions
                                                : this.getParamsOflayoutActionList(
                                                      [subData],
                                                      dataList
                                                  );
                        ...
                        // 新拖出来的组件
                        let appendData = this.getParamsOflayoutActionList(
                            [data],
                            dataList
                        );
                ...
                // 初次保存
                const layoutActionList =
                    this.getParamsOflayoutActionList(dataList);
                const params = {
                    svcId: this.currentServObj?.svcId, // 交易ID
                    layoutActionList,
                    parentActionId: this.fourRAttrFlag
            ...
            return dataList
                ...
                .map((data) => {
                    ...
                    if (
                        ...
                        const subcomponent = JSON.parse(conf.value || "[]");
                        subLayoutActionList = subcomponent.map(
                            (subcomponentId) => {
                                ...
                                return {
                                    ...
                                    actionTyp:
                                        actionTypToArrangeEnum[item.actionType], // 组件类型 1服务2异步组件3并发组件4容器组件5循环组件
                                    parentActionId: data.compositeId, // 父组件ID
                                    loSeqNo: this.getConfItem(
                                        "actionSeq",
                                        item.actionId
                                    ).value, // 位置序号
                                    frntLoSeqNo: this.getConfItem(
                                        "actionFrntSeq",
                                        item.actionId
                                    ).value, // 前置位置序号
                                    postLoSeqNo: this.getConfItem(
                                        "actionPostSeq",
                                        item.actionId
                                    ).value, // 后置位置序号
                                    tfrCdtnCd: "", // 转移条件代码
                    ...
                    return {
                        ...
                        actionKey: this.getConfItem("svcNo", data.actionId)
                            .value, // 服务编号
                        ...
                        loSeqNo: this.getConfItem("actionSeq", data.actionId)
                            .value, // 位置序号
                        frntLoSeqNo: this.getConfItem(
                            "actionFrntSeq",
                            data.actionId
        ...
        async getServiceArrangeList() {
            ...
            console.log('5555555555555layoutActionList', this.layoutActionList)
        },
        // 更新后续编排表
        updateServiceArrangeList(data, currentObj) {
            ...
            if (actionId) {
                const dataList = this.getDataList();
                const subcomponentIdList = dataList.reduce(
                    (prev, container) => {
                        const conf = this.getConfItem(
                            "subcomponent",
                            container.actionId
                ...
                const target = dataList.find(
                    (item) => item.compositeId == actionId
                );
                // 是否更新容器内的组件
                ...
                if (!flag) {
                    ...
                    const target = this.layoutActionList.find((layoutData) => {
                        const subActionList =
                            layoutData.layoutActionList.filter(
                                (item) => item.actionId == actionId
                            );
        ...
        async refreshFlow() {
            ...
            if (this.fourRAttrFlag) {
                ...
                // });
                await this.rpc.newEditor.queryInterfaceProcessLayoutInfo({
                    tranId: this.currentInterfaceObj?.svcId,
                    styleType: this.process_type == '02' ? '00' : '01',
                    parnetActionId: this.currentParentCpnInfo.compositeId,
                }).then(result => {
                    const interfaceProcessActionInfoList = result.interfaceProcessActionInfoList || []
                        // 这里做一层数据转换给到getAllData
                    const res = this.flowDataFormat(interfaceProcessActionInfoList)
                ...
                await Promise.all([
                    ...
                    // 保存当前选中服务信息
                    this.currentServObj = res;
                    const interfaceProcessActionInfoList = _res[2].interfaceProcessActionInfoList || []
                    // 保存最新编排数据
                    this.interfaceProcessActionInfoList = interfaceProcessActionInfoList
                    // 这里做一层数据转换给到getAllData
                    const { flow, position } = this.flowDataFormat(interfaceProcessActionInfoList)
                    res.flow = flow
         ...
         * 编排弹窗的保存
         */
        async conditionalBranchSave(branchInfo) {
            if (!this.handleToSaveVerify()) return;
            this.lineLayoutInfo = branchInfo
            await this.save(true, 'saveParams');
        },
        // 保存按钮 触发保存不返回
        ...
        handleToSaveVerify() {
            ...
            let containerList = this.$refs.process.containerList;
            ...
            // ) {
            //     return (
            //         this.$message.error(`子流程中缺少服务组件，无法保存`) &&
            //         false
            //     );
            // }
            // 校验循环组件的表单填写
            ...
            containerList = containerList.filter((item) => {
                return item.actionType !== "concurrentAction";
            });
            // 校验选择组件的前序组件的交易编排列表不能为空
            ...
            console.log(containerList,frntSeqItemList, "containerList-containerList");
            const unmatchedFrntItem = frntSeqItemList.find((item) => {
                ...
                // );
                // return (
                //     !layoutData?.tfrCdtnCd?.length ||
                //     layoutData?.tfrCdtnCd == "[]"
                // );
            });
        ...
        getOnly(arrs, operate) {
            if (operate == "only") {
                ...
                data.forEach((item) => {
                    const l = list.filter(
                        (it) => it.paramParentId == item.paramParentId
                    );
            ...
            data.map((item) => {
                if (
                    ...
                    if (item.paramDirection in this.descList) {
                        this.descList[item.paramDirection].push(
                            item.paramCName
                        );
                    } else {
        ...
        pcTranform() {
            const k = this.$refs.process.dataList;
            const l = this.$refs.process.lineList.filter(i => !i.isTfrCdtnCd);
            console.log('???????', k, l, this.$refs.process.lineList);
            ...
            const data = JSON.parse(
                JSON.stringify({
                    flow: flow,
                    position: html2Escape(JSON.stringify(p)),
                })
            );
        ...
        async getAllData(val) {
            console.log(this.fourRAttrFlag,val, 'fourRAttrFlag=====')
            if (!val.flow || val.flow.length == 0) {
                ...
                const withoutCompositeIdList = val.flow.filter(
                    (item) => !item.compositeId
                );
                const length = withoutCompositeIdList.length;
                if (length) {
                    ...
                    res.copositeIdList.forEach((compositeId) => {
                        const item = val.flow.find((item) => !item.compositeId);
                        item.compositeId = compositeId;
                    });
                }
        ...
        /**
         * 
         * 根据新的编排关系接口处理组件flow数据
         */
        flowDataFormat(list) {
            let flow = []
            let position = {}
            flow = list.map(e => {
                position[e.actionCd] = `${e.interfaceProcessActionStyleInfo.actionXCoordinate} ${e.interfaceProcessActionStyleInfo.actionYCoordinate} ${e.interfaceProcessActionStyleInfo.actionWidth} ${e.interfaceProcessActionStyleInfo.actionHeight}`
                return {
                    paramList: e.actionParamList,
                    conf: e.interfaceProcessActionConfList.map(conf => {
                        return {
                            name: conf.confKey,
                            value: conf.confValue
                        }
                    }),
                    confList: e.interfaceProcessActionConfList,
                    compositeId: e.actionId,
                    cpntId: e.svcId,
                    id: e.actionCd,
                    referId: e.actionType,
                    type: e.actionType,
                    transition: e.interfaceProcessActionLayoutRelList.map((rel, i) => {
                        let actionCd = rel.endActionType == '01' ? rel.exceptionActionInfo.actionCd : rel.actionCd
                        return {
                            desc: "bottom-top",
                            name: `${e.actionCd}.transition${i}`,
                            polyline: rel.interfaceProcessActionLayoutRelStyleInfo && rel.interfaceProcessActionLayoutRelStyleInfo.coordinateInfoStr,
                            relId: rel.relId,
                            value: actionCd,
                            condition: {
                                ...rel
            ...
            return {
                flow: flow,
                position: html2Escape(JSON.stringify(position))
            }
        },
        ...
        async getServiceDesignDetail() {
            console.log("getServiceDesignDetail")
            // if (!this.currentApiId) return;
            if (!this.currentInterfaceObj.svcId) {
                return;
            } //没有服务不执行
            await Promise.all([
                ...
                const interfaceProcessActionInfoList = _res[2].interfaceProcessActionInfoList || []
                let flowList = []
                if(interfaceProcessActionInfoList.length > 0) {
                    if(interfaceProcessActionInfoList.some(e => e.interfaceProcessActionStyleInfo == null)) {
                        flowList = this.interfaceProcessActionInfoList
                    } else if(this.hasNullLayoutRelActionStyleInfo(interfaceProcessActionInfoList)) {
                        flowList = this.interfaceProcessActionInfoList
                    } else {
                        flowList = interfaceProcessActionInfoList
                        // 保存最新编排数据
                        this.interfaceProcessActionInfoList = interfaceProcessActionInfoList
                    }
                }
                // 这里做一层数据转换给到getAllData
                const { flow, position } = this.flowDataFormat(flowList)
                res.flow = flow
                res.position = position
                // 保存当前选中服务信息
                this.currentServObj = res;
                // await this.getServiceArrangeList();
                this.getAllData(res);
                this.$refs.pattr.currentObj = res;
                this.$refs.pattr.getFlowParams(res);
                this.$nextTick(() => {
         ...
         * 判断是否初始化数据
         */
        hasNullLayoutRelActionStyleInfo(interfaceProcessActionInfoList) {
            return interfaceProcessActionInfoList.some(action => {
                // 获取当前 action 的 interfaceProcessActionLayoutRelList
                const relList = action.interfaceProcessActionLayoutRelList;
                // 检查 relList 是否存在且为数组，然后检查其中是否有 rel 的 styleInfo 为 null
                return Array.isArray(relList) && 
                    relList.some(rel => rel.interfaceProcessActionLayoutRelStyleInfo === null);
            });
        },
        // 遍历数组 重新排序
        getNodeId(arrs, id) {
        ...
        showDialog(e, data) {
            if (e == "group") {
                ...
                this.$refs.createGroupRef.formData.bkId =
                    this.newServiceForm.bkId;
                this.$refs.createGroupRef.formData.arsId =
                    this.newServiceForm.arsId;
        ...
        getConditionDbClickEvent(lineObj, lineList) {
            ...
            let _l = lineList.filter(i => !i.isTfrCdtnCd)
            this.$refs.pattr.getCondition(lineObj, _l);
            // this.mixDic();
        },
        //点击画布空白
        getSvgClickEvent(e) {
        ...
        generateLineCondPrit(lineObj, lineList) {
            let _l = lineList.filter(i => !i.isTfrCdtnCd)
            this.$refs.pattr.generateLineCondPrit(lineObj, _l);
        },
        //元素栏触发
        //元素创建到画布
        pickElemEvent(e) {
        ...
        updateRecordFlow(e) {
            ...
            const updatedItem = _recordFlow.find(
                (item) => item.id == e.actionId
            );
            if (updatedItem) {
        ...
        adjustSize() {
            this.toolsWidth =
                document.querySelector(".process_middle").offsetWidth * 1 -
                10 +
                "px";
        },
        handleMousewheel(e) {
        ...
        deleteLineBySourceIdAndTargetId(sourceId, targetId) {
            this.$refs.process.deleteLineBySourceIdAndTargetId(
                sourceId,
                targetId
            );
        },
        ...
        addService() {
            ...
            this.$refs.demandservListRef.showDialog();
        },
        //选择服务刷新服务列表
        getServiceList() {
            this.$refs.pelem.getServicePickList();
        ...
        interfaceClick(item) {
            ...
            this.$nextTick(() => {
                ...
                if (item.srcFnctFlg == "1") {
                    this.$refs.pelem.showAdd = true;
                    this.isRead = false;
                    this.showSrcFnctFlg = false;
                } else {
                    this.$refs.pelem.showAdd = false;
                    this.isRead = true;
                    this.showSrcFnctFlg = true;
                }
                this.getFlowDetail();
                this.$forceUpdate();
            });
        ...
        //流程图切换
        picClick(e) {
            this.process_type = '02'
            // console.log("picClick", e)
            this.picDefault = e;
        ...
        processTypeChange(val) {
            this.process_type = val
            this.getFlowDetail()
        },
        //主流程业务4b切换
        async getFlowDetail() {
            console.log("getFlowDetail", this.fourRAttrFlag)
            if (this.picDefault == "mainFlow") {
                this.queryMainFlow();
            } else if (this.picDefault == "4bFlow") {
                await this.getServiceDesignDetail();
                this.$nextTick(() => {
                    // console.log("this.layoutActionList", JSON.parse(JSON.stringify(this.layoutActionList)))
                    // this.$refs.process?.handleProcessModeChange("complex");
        ...
        async updataServ(type, data) {
            if (type == "update") {
                if (this.currentInterfaceObj.addState == "2")
                    this.isRead = true;
                else this.isRead = false;
