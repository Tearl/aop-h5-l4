        ...
        <div class="content">
          <div class="table_operate">
            <div class="header_content">
              <h1>服务列表</h1>
              <div class="list_type" v-if="changeTabs=='asset'">
                <el-button
                    class="item"
                    v-for="(item, index) in funcList"
                    :key="index"
                    :class="getClass(item)"
                    @click="classTagClick(item.value)"
                    size="small">{{item.label}}</el-button>
                <el-dropdown class="item_dropdown" @command="tagCommand" trigger="click" v-if="funcList.length>6">
                    <span class="el-dropdown-link">
                        更多<i class="el-icon-caret-bottom"></i>
                    </span>
                    <el-dropdown-menu slot="dropdown">
                        <el-dropdown-item
                        v-for="(item, index) in funcList2"
                        :key="index"
                        :command="['classTagClick', item.value]"
          ...
          <Table
            ...
            class="table_style"
            :table="changeTabs=='asset'?tables2:tables"
            :data="dictList"
            :rowK="'serialNumber'"
  ...
  methods: {
    ...
    async pageApiInfo(){
        const params = {
            ...
            assetCode: this.$route.query.fnctNo || '', //活动编号
            funcId: this.functionId || '',
            tranId: "", //交易id
            keyWord: this.searchRecord.apiNuCoNm || '', //服务编码或名称
            currentPage: this.pager.currentPage + "",
            turnPageShowNum: this.pager.turnPageShowNum,
      };
      const res = await this.rpc.s4design.pageApiInfo(params);
      this.dictList = res.decisionTreeAndApiListBos;
      this.pager.total = res.turnPageTotalNum * 1;
      this.$nextTick(() => {
        this.isTableData();
      });
    ...
    getClass(item) {
        return this.functionId == item.value
            ? 'list_active'
            : 'list_item'
    },
    async classTagClick(val) {
        this.functionId = val;
        await this.pageApiInfo();
    },
    async tagCommand(e){
        if(["classTagClick"].includes(e[0])){ //单元测试预览
            this.classTagClick(e[1]);
        }
    },
    async getFucList() {
        let params = {
        ...
        this.funcList = [];
        let res = await this.rpc.s4design.getAssetsFunctionInfoList(
            params
        );
        let arr = res.functionInfoList || [];
        ...
        let arr3 = [
            ...
            this.funcList = newList;
            this.funcList2 = newList2;
    },
    // 数据字典选择每页条数改变时
    handleSizeChange(e) {
      this.isRecord = true;
