  ...
  <div class="aop_tradecode_comps_d4editor_service_define_check_dialog">
    ...
    <PageDialog :dialogWidth="'720px'" :dialogTitle="'详情'" :dialogVisiable="standardCheckDetailVisible"
      ...
      <div slot="box">
        ...
        <div v-if="currentShowTitle == '12'">
          <span class="title">组件配置信息保存列表：</span>
          <Table class="table" :data="actionConfigInfoCommitResultList" :table="actionColumn" @operate="operate"></Table>
        </div>
        <div v-if="currentShowTitle == '13'">
          <span class="title">组件编排规则列表：</span>
          <Table class="table" :data="actionLayoutInfoExistResultList" :table="actionColumn" @operate="operate"></Table>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      // 组件列表
      actionConfigInfoCommitResultList: [], //组件配置信息保存列表
      actionLayoutInfoExistResultList: [], // 组件编排规则列表
  ...
  methods: {
    ...
    async showStandardCheckDialog(data) {
      ...
      await setTimeout(() => {
        ...
        this.standardCheckData = [...this.standardCheckData]
      }, 500)
      await setTimeout(() => {
        // 接口绑定交互事件检测
      ...
      await setTimeout(() => {
        ...
        this.standardCheckData[4].checkFlg = data.txInputParamUsedInLayoutCheck
        this.standardCheckData = [...this.standardCheckData]
      }, 3000)
      await setTimeout(() => {
        // 接口输出参数未被完全赋值检测
        this.setParamList = data.txOutputParamBeAssignResultList
        this.standardCheckData[5].checkFlg = data.txOutputParamBeAssignCheck
        console.log(data.txOutputParamBeAssignCheck, 'txOutputParamBeAssignCheck')
        this.standardCheckData = [...this.standardCheckData]
      }, 3500)
      await setTimeout(() => {
        // 接口输出参数来源检测
        this.sourceParamList = data.txOutputParamSrcIdResultList
        this.standardCheckData[6].checkFlg = data.txOutputParamSrcIdCheck
        this.standardCheckData = [...this.standardCheckData]
      }, 4000)
      await setTimeout(() => {
        // 组件配置信息保存检测
        this.actionConfigInfoCommitResultList = this.actionNameJoin(data.actionConfigInfoCommitResultList)
        this.standardCheckData[7].checkFlg = data.actionConfigInfoCommitCheck
        this.standardCheckData = [...this.standardCheckData]
      }, 4500)
      await setTimeout(() => {
        // 组件编排规则检测
        this.actionLayoutInfoExistResultList = this.actionNameJoin(data.actionLayoutInfoExistResultList)
        this.standardCheckData[8].checkFlg = data.actionLayoutInfoExistCheck
        this.standardCheckData = [...this.standardCheckData]
      }, 5000)

      await setTimeout(() => {
        // 组件输入参数未被完全赋值检测
        this.actionParamUsedResultList = this.actionNameJoin(data.actionIsNeedInputParamBeAssginResultList)
        this.standardCheckData[9].checkFlg = data.actionIsNeedInputParamBeAssginCheck
        this.standardCheckData = [...this.standardCheckData]
      }, 5500)
      await setTimeout(() => {
        // 组件输出参数引用检测
        this.paramCheckList = this.actionNameJoin(data.actionOutputParamBeUserOneResultList)
        this.standardCheckData[10].checkFlg = data.actionOutputParamBeUserOneCheck
        this.standardCheckData = [...this.standardCheckData]
      }, 6000)
      await setTimeout(() => {
        // L5服务未被完全编排检测
        this.l5ParamList = data.l5serviceBeUserInLayoutResultList
        this.standardCheckData[11].checkFlg = data.l5serviceBeUserInLayoutCheck
        this.standardCheckData = [...this.standardCheckData]
      }, 6500)
      await setTimeout(() => {
        // 组件输入参数来源检测
        this.tranParamSourceResultList = this.actionNameJoin(data.actionOutputParamSrcIdResultList)
        this.standardCheckData[12].checkFlg = data.actionOutputParamSrcIdCheck
        this.checkEnd = true
   ...
   actionNameJoin(list) {
    return list.map(e => {
      return {
        name: `${e.actionName}(${e.actionSeqNo})`,
        ...e
      }
    })
   },
