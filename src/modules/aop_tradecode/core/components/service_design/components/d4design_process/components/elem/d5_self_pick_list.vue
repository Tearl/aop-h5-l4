    ...
    <ul class="api_info_list">
      <li v-for="(item, index) in apiInfoList" :key="item" class="api_info_list_item" :class="{changeColor:item.changeTp == '1',upgradesColor:item.changeTp == '2'}">
        ...
        <div v-if="!isRead" class="right">
          ...
          <!-- <i class="el-icon-delete-solid" @click.stop="del(item)"></i> -->
          <el-tooltip class="item" effect="dark" content="删除" placement="top" v-if="$route.query.displayType != '01'">
            <i class="el-icon-delete-solid" @click.stop="del(item)"></i>
          </el-tooltip>
...
export default {
  ...
  methods: {
    ...
    async getAssociatedSelfServiceList() {
      ...
      console.log("接口设计-服务列表=====>",this.currentInterfaceObj)
      const params = {
        tranId: this.currentInterfaceObj && this.currentInterfaceObj.svcId,
      };
      const paramsChange = {
      ...
      const res = await this.rpc.d4.queryWaitUseServiceList(params);
      this.apiInfoList = res.waitUseServiceList.map((item) => {
        item.apiName = item.svcNm
        item.menuName = item.svcNm;
        item.menuId = item.svcId;
        item.leaf = true;
        item.svcId = item.svcId;
        item.svcNm = item.svcNm;
        item.svcNo = item.svcCd
        // item.svcNo = item.svcVerNo;
        item.svcTpCd = 'D5自有服务';
        item.apiServiceType = 'D5自有服务'
        // item.sysNm = item.relySysName;
        let sameItem = resChangeServ?.serviceChangeList.length> 0 && resChangeServ?.serviceChangeList.find(someItem =>someItem.apiId == item.svcId)
        if(sameItem){
    ...
    async del(data) {
      this.$confirm("是否删除该服务，服务名称为" + data.apiName)
        .then(async () => {
          const params = {
            svcIdList: [data.svcId],
            tranId: this.currentInterfaceObj.svcId,
          };
          const res = await this.rpc.newEditor.removeWaitUseService(params);
          this.$notify({
            title: "成功",
            message: "删除成功！",
            duration: 2000,
