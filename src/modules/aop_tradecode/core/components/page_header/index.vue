<template>
  <div class="aop_developer_comps_page_header">
      ...
      <div class="header">
        ...
        >
          <template>
            ...
            <a v-if="!!item.path" @click="pathClick(item)">{{rName || item.label}}</a>
            <span v-if="!item.operate & !item.path">{{item.label}}</span>
          </template>
        </span>
      </div>
...
export default {
  ...
  data() {
    return {
      rName: this.$route.query.rName ||  "", // bread name
      rId: this.$route.query.rId || "" // path id
    };
  },
  methods: {
...
.aop_developer_comps_page_header {
  height: 48px;
  .header_wrapper {
    position: fixed;
    width: calc(100%);
