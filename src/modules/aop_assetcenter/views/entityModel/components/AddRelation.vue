<template>
  <div>
    <el-dialog
      :title="showType == 'add' ? '添加对象关系' : '编辑对象关系'"
      :visible.sync="visible"
      class="object-graph-relation-dialog"
      width="720px"
    ...
    >
      <div class="dialog-content" v-if="visible">
        ...
        <div class="center">
          ...
          <div class="relations-select">
            ...
            >
              ...
              <template v-else>
                <el-option
                  v-for="(item, index) in relationArray00"
                  :key="index"
                  :label="item.label"
                  :value="item.value"
                ></el-option>
            ...
            <template v-if="relationForm.obrRelash == 'extends'">
              ...
              >
                <el-option
                  v-for="item in versionArray_extends"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                ></el-option>
            ...
            <template v-if="relationForm.obrRelash == 'combination' || relationForm.obrRelash == 'association'">
              <el-select
                v-model="relationForm.relashMult"
                style="padding-top: 8px"
                width="128"
                ...
                @change="relaChange"
                ><el-option
                  v-for="item in versionArray_combination"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                ></el-option>
            ...
            <template v-if="relationForm.obrRelash == 'aggregation'">
              ...
              >
                <el-option
                  v-for="item in versionArray_aggregation"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                ></el-option
          ...
          <div class="target-obj">
            ...
            <el-select
              :disabled="relationType == 'line'"
              filterable
              style="margin-top: 16px; width: 128px"
              size="small"
              @change="targetChange"
              v-model="targetObjObjId"
            >
              <el-option
                v-for="item in targetObjList"
                :key="item.objId"
                :label="item.objName"
                :value="item.objId"
              ></el-option
        ...
        <el-form ref="base" :model="relationForm" :rules="rules" size="small">
          <el-form-item label="关系名称" prop="obrName">
            <el-input
              maxLength="80"
              v-model.trim="relationForm.obrName"
              placeholder="请输入关系名称"
            />
          </el-form-item>
          <el-form-item label="编码" prop="obrCode">
            <el-input
              maxLength="80"
              v-model.trim="relationForm.obrCode"
              placeholder="请输入编码"
            />
          </el-form-item>
          <el-form-item label="描述" prop="obrDesc">
            <el-input
              v-model.trim="relationForm.obrDesc"
              placeholder="请输入描述"
            />
          </el-form-item>
        </el-form>
      ...
      <div class="btn-group" slot="footer">
        ...
        <el-button size="medium" type="danger" @click="deleteRelation" v-if="showType == 'edit'">删除</el-button>
...
export default {
  ...
  methods: {
      ...
      this.$refs.base.validate((valid) => {
        if (valid) {
          ...
          this.relationForm = {
            ...
            edgePropsId: this.relation ? this.relation.edgePropsId : "",
          };
          this.$emit(
            "relation",
            this.relationForm,
    ...
    deleteRelation() {
      this.$confirm("确定删除该关系吗？", "删除提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(async () => {
          const { obrId } = this.relation;
          await this.rpc.assetcenter.delBizObjRel({ obrId });
          this.$message.success("删除成功!");
          this.$emit("delRelation", this.edge);
          this.close();
